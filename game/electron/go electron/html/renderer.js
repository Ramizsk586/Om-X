document.addEventListener('DOMContentLoaded', () => {
    ...
    async function engineMove() {
        ...
        if (window.electronAPI.goGameStateUpdated) {
            window.electronAPI.goGameStateUpdated(brain.getGameRecord());
        }
    }
    ...
    async function init() {
        ...
        function navigateTo(viewId, url) {
            ...
            if (viewId === 'home') {
                ...
                // Use a webview for isolation and simple navigation
                const correctedUrl = `../html/${url}`;
                const preloadPathForUrl = mainPreloadPath.replace(/\\/g, '/');
                // Re-create webview to ensure clean state
        ...
        document.getElementById('nav-review').addEventListener('click', () => {
            navigateTo('review', 'review.html'); // Just navigate, the review page pulls data itself now
        });
        ...
        if (window.electronAPI && window.electronAPI.onLoadReviewPanel) {
            window.electronAPI.onLoadReviewPanel(gameData => {
                ...
                // We no longer manually push data here to the webview to avoid race conditions.
                // The review panel will pull it on load via `getLastGame`.
            });
        }
    }
