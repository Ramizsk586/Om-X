...
export async function toolsProvider(ctl:ToolsProviderController):Promise<Tool[]> {
	...
	const extractWebResults = (html: string, limit: number) => {
		...
		const push = (label = "", url = "", snippet = "") => {
			...
			seen.add(cleanUrl);
			const cleanLabel = String(label || "Untitled").replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim() || "Untitled";
			const cleanSnippet = String(snippet || "").replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim();
			links.push([cleanLabel, cleanUrl, cleanSnippet]);
		};
		...
		if (links.length === 0) {
			const regex = /<a class="result__a" href="([^"]+)">([^<]*)<\/a>/gm;
	...
	const duckDuckGoWebSearchTool = tool({
		...
		implementation: async ({ query, pageSize, safeSearch, page }, { status, warn, signal }) => {
			...
			try {
				...
				// Construct DuckDuckGo HTML endpoint
				...
				for (let attempt = 1; attempt <= 2; attempt++) {
					...
					if (!response.ok) {
						lastErr = response.statusText || `HTTP ${response.status}`;
						if (attempt < 2) {
							await new Promise(resolve => setTimeout(resolve, 600 * attempt));
							continue;
						}
						warn(`Failed to fetch search results: ${lastErr}`);
						return `Error: Failed to fetch search results: ${lastErr}`;
					}
					const html = await response.text();
					const parsed = extractWebResults(html, pageSize);
					...
					status(`Found ${parsed.length} web pages.`);
					return {
						links: parsed.map(([label, resultUrl]) => [label, resultUrl]),
						results: parsed.map(([label, resultUrl, snippet]) => ({ title: label, url: resultUrl, snippet })),
						count: parsed.length,
					};
				}
				return `Error: Failed to fetch search results: ${lastErr}`;
			...
			try {
				...
				const downloadPromises = imageURLs.map(async (url: string, i: number) => {
					...
					try {
                        ...
                        // Use arrayBuffer() and convert to Uint8Array as bytes() might not be available in all environments
