...
function isPathInSafeDirectories(targetPath) {
  try {
    ...
    // Add system/environment directories
    safeDirectories.push(path.resolve(os.tmpdir()));
    if (process.env.USERPROFILE) safeDirectories.push(path.resolve(process.env.USERPROFILE));
    if (process.env.HOME) safeDirectories.push(path.resolve(process.env.HOME));
    
    // Also allow the app's base directory and parent
    safeDirectories.push(path.resolve(app.getAppPath()));
    try { safeDirectories.push(path.resolve(app.getPath('appData'))); } catch (e) {}
