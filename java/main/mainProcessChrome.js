...
// Dedicated process entry for Chrome-style UI mode.
process.env.OMX_MAIN_PROCESS_VARIANT = 'chrome';
...
const BUILTIN_EXTENSION_POLICIES = Object.freeze({
  'uBlock-master': Object.freeze({
    enabledByDefault: true,
    runtimeCandidates: Object.freeze([
      path.join('.omx-runtime-chromium'),
      path.join('platform', 'chromium'),
      path.join('platform', 'mv3', 'chromium'),
      '.'
    ]),
    allowedManifestVersions: new Set([2, 3]),
    blockedPermissions: new Set(['nativeMessaging', 'management', 'proxy'])
  })
});
...
// Coder DevTools must remain disabled for end users.
const TEMP_CODER_AUTO_OPEN_DEVTOOLS = false;
    ...
    for (const trustedDir of trustedFolders) {
      try {
        if (trustedDir && isSameOrSubFsPath(abs, trustedDir)) {
          console.log('[Security] Path allowed via trusted folder:', abs);
          return true;
        }
      } catch (e) {}
    }
    ...
    for (const safeDir of safeDirectories) {
      try {
        if (safeDir && isSameOrSubFsPath(abs, safeDir)) {
          console.log('[Security] Path allowed via default safe directory:', abs);
          return true;
        }
      } catch (e) {}
    }
...
function getFilePathFromArgs(args) {
    const skip = [app.getPath('exe'), '.', './', 'index.js', 'main.js', 'java/main/mainProcess.js', 'java/main/mainProcessChrome.js'];
...
function normalizeBrowserUiMode(mode) {
  return String(mode || '').trim().toLowerCase() === 'chrome' ? 'chrome' : 'default';
}

function getMainWindowHtmlPath(settings = cachedSettings) {
  const forcedVariant = String(process.env.OMX_MAIN_PROCESS_VARIANT || '').trim().toLowerCase();
  const mode = forcedVariant === 'chrome'
    ? 'chrome'
    : normalizeBrowserUiMode(settings?.browserUI?.mode);
  const fileName = mode === 'chrome' ? 'main-chrome.html' : 'main.html';
  return path.join(__dirname, '../../html/windows', fileName);
}
...
try {
  if (fs.existsSync(settingsPath)) {
    ...
    cachedSettings = {
      ...
      browserUI: {
        ...DEFAULT_SETTINGS.browserUI,
        ...(saved?.browserUI || {}),
        mode: normalizeBrowserUiMode(saved?.browserUI?.mode)
      },
      blocklist: mergeWithDefaultBlocklist(saved?.blocklist)
    };
  }

  // load any enabled extensions now that settings are available
  applyExtensions(cachedSettings).catch(e => console.warn('[Extensions] initial load failed', e));
...
async function prepareBuiltInUBlockRuntime(sourceDir) {
    const sourceAbs = path.resolve(String(sourceDir || ''));
    const runtimeDir = path.join(sourceAbs, '.omx-runtime-chromium');
    const runtimeManifestPath = path.join(runtimeDir, 'manifest.json');
    const runtimeBgPath = path.join(runtimeDir, 'background.html');
    const runtimeWebextPath = path.join(runtimeDir, 'js', 'webext.js');
    const runtimeVapiExtPath = path.join(runtimeDir, 'js', 'vapi-background-ext.js');
    const sanitizeRuntimeManifest = async () => {
        try {
            const manifestRaw = await fs.promises.readFile(runtimeManifestPath, 'utf8');
            const manifest = JSON.parse(manifestRaw);
            const knownElectronMv2Permissions = new Set([
                'alarms',
                'storage',
                'tabs',
                'unlimitedStorage',
            ...
            if (Array.isArray(manifest.permissions)) {
                manifest.permissions = manifest.permissions
                    .map((p) => String(p || '').trim())
                    .filter(Boolean)
                    .filter((p) => p.startsWith('http://') || p.startsWith('https://') || p.startsWith('*://') || knownElectronMv2Permissions.has(p));
            }
            await fs.promises.writeFile(runtimeManifestPath, JSON.stringify(manifest, null, 2), 'utf8');
        } catch (e) {
            console.warn('[Extensions] Failed to sanitize uBlock runtime manifest:', e?.message || e);
        }
    };

    // Fast path if runtime already looks complete.
    if (
        await fileExists(runtimeManifestPath) &&
        await fileExists(runtimeBgPath) &&
        await fileExists(runtimeWebextPath) &&
        await fileExists(runtimeVapiExtPath)
    ) {
        await sanitizeRuntimeManifest();
        return runtimeDir;
    }

    const srcRoot = path.join(sourceAbs, 'src');
    const chromiumRoot = path.join(sourceAbs, 'platform', 'chromium');
    const commonRoot = path.join(sourceAbs, 'platform', 'common');
    if (!(await fileExists(path.join(srcRoot, 'background.html')))) {
        throw new Error('uBlock source background.html was not found.');
    }
    if (!(await fileExists(path.join(chromiumRoot, 'manifest.json')))) {
        throw new Error('uBlock chromium manifest was not found.');
    }

    await ensureCleanDir(runtimeDir);
    await copyDirectoryRecursive(srcRoot, runtimeDir);

    await copyFileIfPresent(path.join(chromiumRoot, 'manifest.json'), path.join(runtimeDir, 'manifest.json'));
    await copyFileIfPresent(path.join(chromiumRoot, 'webext.js'), path.join(runtimeDir, 'js', 'webext.js'));
    await copyFileIfPresent(path.join(chromiumRoot, 'vapi-background-ext.js'), path.join(runtimeDir, 'js', 'vapi-background-ext.js'));
    await copyFileIfPresent(path.join(chromiumRoot, 'is-webrtc-supported.js'), path.join(runtimeDir, 'is-webrtc-supported.js'));
    await copyFileIfPresent(path.join(chromiumRoot, 'is-webrtc-supported.html'), path.join(runtimeDir, 'is-webrtc-supported.html'));

    await copyFileIfPresent(path.join(commonRoot, 'vapi.js'), path.join(runtimeDir, 'js', 'vapi.js'));
    await copyFileIfPresent(path.join(commonRoot, 'vapi-client.js'), path.join(runtimeDir, 'js', 'vapi-client.js'));
    await copyFileIfPresent(path.join(commonRoot, 'vapi-common.js'), path.join(runtimeDir, 'js', 'vapi-common.js'));
    await copyFileIfPresent(path.join(commonRoot, 'vapi-background.js'), path.join(runtimeDir, 'js', 'vapi-background.js'));
    await copyFileIfPresent(path.join(commonRoot, 'managed_storage.json'), path.join(runtimeDir, 'managed_storage.json'));

    // Electron supports only a subset of MV2 permissions.
    await sanitizeRuntimeManifest();
    ...
    if (
        ...
        throw new Error('uBlock runtime assembly is incomplete.');
    }

    return runtimeDir;
}

async function resolveBuiltInExtensionFolder(extensionName, sourceDir) {
    const name = String(extensionName || '').trim();
    if (name === 'uBlock-master') {
        return prepareBuiltInUBlockRuntime(sourceDir);
    }
    return null;
}
...
function validateExtensionSafety(extensionName, sourceDir, resolvedDir, manifestMeta) {
    ...
    const hasAllUrls = permissions.includes('<all_urls>') || hostPermissions.includes('<all_urls>');
    if (!hasAllUrls) {
        return { safe: false, error: 'Built-in ad blocker must request <all_urls> access.' };
    }

    if (manifestVersion === 2 && !permissions.includes('webRequest')) {
        return { safe: false, error: 'Manifest v2 built-in ad blocker must request webRequest permission.' };
    }

    if (manifestVersion === 3 && !permissions.includes('declarativeNetRequest')) {
        return { safe: false, error: 'Manifest v3 built-in ad blocker must request declarativeNetRequest permission.' };
    }
...
async function resolveExtensionFolder(dir, options = {}) {
    ...
    const isManifestRuntimeUsable = async (manifestDir) => {
        try {
...
async function ensureExtensionLoaded(dir, extensionName = '') {
    try {
        let realDir = await resolveExtensionFolder(dir, { prepareBuild: false });
        if (!realDir && getBuiltinExtensionPolicy(extensionName)) {
            try {
                realDir = await resolveBuiltInExtensionFolder(extensionName, dir);
            } catch (e) {
        ...
        const ext = await session.defaultSession.loadExtension(realDir, { allowFileAccess: false });
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (shouldEnable && !already) {
                ...
                try {
                    session.defaultSession.removeExtension(already.id);
                } catch (e) {}
...
function toggleDevTools(win) {
    ...
    if (coderWindow && !coderWindow.isDestroyed() && target === coderWindow) return;
...
function attachCoderContextMenu(hostWindow) {
  if (!hostWindow || hostWindow.isDestroyed()) return;
  const sourceContents = hostWindow.webContents;
  if (!sourceContents || sourceContents.isDestroyed?.()) return;

  sourceContents.on('context-menu', (event, params = {}) => {
    event.preventDefault();
    const hasTextSelection = Boolean(params.selectionText && String(params.selectionText).trim().length > 0);
    const canPaste = Boolean(params.editFlags?.canPaste);
    const canUndo = Boolean(params.editFlags?.canUndo);
    const canRedo = Boolean(params.editFlags?.canRedo);
    const editable = Boolean(params.isEditable);

    const template = [
      { role: 'undo', label: 'Undo', enabled: canUndo },
      { role: 'redo', label: 'Redo', enabled: canRedo },
      { type: 'separator' },
      { role: 'cut', label: 'Cut', enabled: editable && hasTextSelection },
      { role: 'copy', label: 'Copy', enabled: hasTextSelection },
      { role: 'paste', label: 'Paste', enabled: editable && canPaste },
...
function createAIPlayerWindow() {
  ...
  aiPlayerWindow.loadFile(path.join(__dirname, '../../html/pages/minecraft.html'), {
    query: { mode: 'ai', panel: 'ai-player' }
  });
  attachMiniAppContextMenu(aiPlayerWindow, { includeNavigation: false });
...
function createAIChatWindow() {
  ...
  attachMiniAppContextMenu(aiChatWindow, { includeNavigation: false });
...
function createCanvasWindow() {
  ...
  attachMiniAppContextMenu(canvasWindow, { includeNavigation: false });
...
function createImageEditorWindow() {
  ...
  attachMiniAppContextMenu(imageEditorWindow, { includeNavigation: false });
...
function createCoderWindow() {
  ...
  attachCoderContextMenu(coderWindow);
  ...
  coderWindow.webContents.once('did-finish-load', () => {
    ...
    if (!wcId) return;
    if (TEMP_CODER_AUTO_OPEN_DEVTOOLS) {
      try {
        coderWindow?.webContents?.openDevTools({ mode: 'detach' });
      } catch (_) {}
    }
  });
...
function createMinecraftGameWindow() {
    ...
    minecraftGameWindow.loadURL('mindclone://app/game/electron/mindclone/minecraft.html');
    attachMiniAppContextMenu(minecraftGameWindow, { includeNavigation: false });
...
ipcMain.handle('app-restart', async () => {
    const targetMode = normalizeBrowserUiMode(cachedSettings?.browserUI?.mode);
    const passthroughArgs = process.argv.slice(1).filter((arg) => !String(arg || '').startsWith('--main-process='));
    const nextArgs = targetMode === 'chrome'
      ? [...passthroughArgs, '--main-process=chrome']
      : passthroughArgs;
...
function isPrivilegedSettingsSender(event) {
  ...
  try {
    ...
    return pathname.endsWith('/html/windows/system.html')
      || pathname.endsWith('/html/pages/ai-settings.html');
  } catch (_) {
    return false;
  }
}
...
function preserveSensitiveSettings(currentSettings, nextSettings) {
  ...
  const currentVtKey = String(current?.security?.virusTotal?.apiKey || '');
  if (!preserved.security) preserved.security = {};
  if (!preserved.security.virusTotal) preserved.security.virusTotal = {};
  preserved.security.virusTotal.apiKey = currentVtKey;
...
ipcMain.handle('extensions:list', async () => {
  ...
  try {
    ...
    for (const ent of entries) {
      ...
      const name = ent.name;
      const sourcePath = path.join(extDir, name);
      let resolvedPath = await resolveExtensionFolder(sourcePath, { prepareBuild: false });
      if (!resolvedPath && getBuiltinExtensionPolicy(name)) {
        try { resolvedPath = await resolveBuiltInExtensionFolder(name, sourcePath); } catch (_) {}
      }
      const manifestMeta = resolvedPath ? await readExtensionManifestMeta(resolvedPath) : null;
      const enabled = cachedSettings.extensions?.[name]?.enabled === true;
      const loaded = !!loadedExtensions[name];
      const error = extensionErrors[name] || (!manifestMeta ? 'manifest.json not found in extension folder/build output.' : null);

      result.push({
        id: loadedExtensions[name]?.id || null,
        name,
        displayName: manifestMeta?.displayName || name,
        description: manifestMeta?.description || '',
        version: manifestMeta?.version || '',
        manifestVersion: manifestMeta?.manifestVersion ?? null,
        icon: manifestMeta?.icon || '',
        enabled,
        loaded,
        path: sourcePath,
        runtimePath: resolvedPath || sourcePath,
        error,
        hasManifest: !!manifestMeta,
        manifestPath: manifestMeta?.manifestPath || null
      });
    }
  } catch (e) {
    // ignore read errors
...
ipcMain.handle('extensions:toggle', async (_event, extensionName) => {
  const name = String(extensionName || '').trim();
  if (!name) return { enabled: false, error: 'Invalid extension name.' };

  const currentlyEnabled = cachedSettings.extensions?.[name]?.enabled === true;
  ...
  const nextSettings = {
    ...
    extensions: {
      ...
      [name]: {
        ...(cachedSettings.extensions?.[name] || {}),
        enabled: nextEnabled
      }
    }
  };

  const success = await safeWriteJson(settingsPath, nextSettings);
  if (!success) {
    return { enabled: currentlyEnabled, error: 'Failed to save extension settings.' };
  }

  cachedSettings = nextSettings;
  configureSecurity(cachedSettings, mainWindow);
  await applyExtensions(cachedSettings);
  broadcast('settings-updated', cachedSettings);

  return {
    enabled: nextEnabled,
    loaded: !!loadedExtensions[name],
    error: extensionErrors[name] || null
  };
});
...
ipcMain.handle('settings-save', async (e, s) => {
  ...
  const normalizedSettings = {
    ...
    browserUI: {
      ...DEFAULT_SETTINGS.browserUI,
      ...(cachedSettings?.browserUI || {}),
      ...(incoming?.browserUI || {}),
      mode: normalizeBrowserUiMode(incoming?.browserUI?.mode ?? cachedSettings?.browserUI?.mode)
    },
    blocklist: mergeWithDefaultBlocklist(incoming?.blocklist ?? cachedSettings?.blocklist)
  };
...
ipcMain.handle('system-get-info', () => {
  const FALLBACK_DEP_VERSIONS = {
    ...
    electronUpdater: '6.8.3'
  };

  const tryReadJson = (candidatePath) => {
    try {
  ...
  let electronBuilderInstalled = '';
  let electronUpdaterInstalled = '';
  try { electronBuilderInstalled = require('electron-builder/package.json')?.version || ''; } catch (_) {}
  try { electronUpdaterInstalled = require('electron-updater/package.json')?.version || ''; } catch (_) {}
  ...
  for (const candidate of packageCandidates) {
    packageData = tryReadJson(candidate);
    if (packageData) break;
  }

  const dependencies = {
    electronBuilder:
      electronBuilderInstalled
      || packageData?.devDependencies?.['electron-builder']
      || packageData?.dependencies?.['electron-builder']
      || FALLBACK_DEP_VERSIONS.electronBuilder,
    electronUpdater:
      electronUpdaterInstalled
      || packageData?.dependencies?.['electron-updater']
      || packageData?.devDependencies?.['electron-updater']
      || FALLBACK_DEP_VERSIONS.electronUpdater
  };
...
ipcMain.handle('fs-trust-folder', async (e, folderPath) => {
    try {
        ...
        console.log('[Security] Trusted folder registered:', abs);
...
ipcMain.handle('fs-read-dir', async (e, dirPath) => {
    try {
        const abs = path.resolve(String(dirPath || ''));
        console.log('[Security] fs-read-dir request for:', abs);
        console.log('[Security] Trusted folders:', Array.from(trustedFolders));
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('coder:window-get-terminals', async (event) => {
    try {
      ensureCoderSenderWindow(event);
      const ownerWebContentsId = Number(event?.sender?.id) || 0;
      if (!ownerWebContentsId) return [];
      const terminals = [];
      for (const session of coderTerminalSessions.values()) {
        if (!session || Number(session.ownerWebContentsId) !== ownerWebContentsId) continue;
        terminals.push({
          id: String(session.id || ''),
          cwd: String(session.cwd || ''),
          isRunning: session.ended !== true,
          pid: Number(session.child?.pid) || null
        });
      }
      return terminals;
    } catch (_) {
  ...
  ipcMain.handle('coder:window-get-active-terminal', async (event) => {
    try {
      ensureCoderSenderWindow(event);
      const ownerWebContentsId = Number(event?.sender?.id) || 0;
      if (!ownerWebContentsId) return null;
      let active = null;
      for (const session of coderTerminalSessions.values()) {
        if (!session || Number(session.ownerWebContentsId) !== ownerWebContentsId) continue;
        if (session.ended === true) continue;
        active = {
          id: String(session.id || ''),
          cwd: String(session.cwd || ''),
          isRunning: true,
  ...
  ipcMain.handle('coder:window-get-visible-notebook-editors', async (event) => {
    try { ensureCoderSenderWindow(event); } catch (_) { return []; }
    return [];
  });

  ipcMain.handle('coder:window-get-active-notebook-editor', async (event) => {
    try { ensureCoderSenderWindow(event); } catch (_) { return null; }
    return null;
  });
