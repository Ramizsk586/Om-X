...
class AIGuardian {
  ...
  getPrimaryIdentity(hostname) {
    ...
    // Normalize full URLs / host:port / path-like inputs into hostname.
    try {
        if (/^[a-z][a-z0-9+.-]*:\/\//i.test(domain)) {
            domain = new URL(domain).hostname.toLowerCase();
        } else if (domain.includes('/') || domain.includes('?') || domain.includes('#')) {
            domain = new URL(`https://${domain.replace(/^\/\//, '')}`).hostname.toLowerCase();
        }
    } catch (_) {}
