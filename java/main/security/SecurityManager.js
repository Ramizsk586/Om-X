...
class SecurityManager {
  ...
  isPrimaryAdblockExtensionEnabled() {
    // If the built-in adblock extension is explicitly disabled, disable
    // Om-X network ad/tracker filtering so toggle behavior is consistent.
    return this.settings?.extensions?.['uBlock-master']?.enabled !== false;
  }

  buildAdblockRules() {
    const cfg = this.settings?.adBlocker || {};
    const enabled = cfg.enabled !== false;
    if (!enabled || !this.isPrimaryAdblockExtensionEnabled()) {
      this.adblock = { rules: [], regexes: [], allowlist: new Set() };
      return;
    }
  ...
  isLikelyYouTubeAdMediaRequest(urlObj) {
    try {
      const hostname = String(urlObj?.hostname || '').toLowerCase();
      const pathname = String(urlObj?.pathname || '').toLowerCase();
      if (!hostname.endsWith('googlevideo.com') || !pathname.includes('/videoplayback')) return false;
      ...
      for (const key of adMarkers) {
        if (q.has(key)) return true;
      }
      const ctier = String(q.get('ctier') || '').toUpperCase();
      if (ctier === 'A') return true;

      const mime = String(q.get('mime') || '').toLowerCase();
      const source = String(q.get('source') || '').toLowerCase();
      if (mime.includes('video/mp2t') && source.includes('yt_ads')) return true;
    } catch (_) {}
    return false;
  }
  ...
  shouldBlockAd(details) {
    ...
    // Allow core YouTube media by default, but explicitly block ad-marked
    // `videoplayback` requests to improve ad suppression without breaking playback.
    if (hostname.endsWith('googlevideo.com') && pathname.includes('/videoplayback')) {
      return this.isLikelyYouTubeAdMediaRequest(urlObj);
    }
