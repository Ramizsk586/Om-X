...
import { GrammarUI } from './ui/grammar.js';
import { GrammarTranslateUI } from './ui/grammarTranslate.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const EXTENSIONS_URL = new URL('../../html/windows/extensions.html', import.meta.url).href;
  ...
  const BOOKMARKS_URL = new URL('../../html/pages/bookmarks.html', import.meta.url).href;
  const MINI_APPS_URL = new URL('../../html/pages/mini-apps.html', import.meta.url).href;
  ...
  const measureMenuRect = (menuEl) => {
      ...
      menuEl.style.left = original.left;
      menuEl.style.top = original.top;
      menuEl.style.visibility = original.visibility;
      if (wasHidden) menuEl.classList.add('hidden');

      return {
          width: rect.width || menuEl.offsetWidth || 0,
          height: rect.height || menuEl.offsetHeight || 0
      };
  };

  const placeMenuAtPoint = (menuEl, x, y, options = {}) => {
      if (!menuEl) return;
      const padding = Number.isFinite(options.padding) ? options.padding : 8;
      const preferVertical = options.preferVertical || 'auto';
      const { width, height } = measureMenuRect(menuEl);

      const maxLeft = Math.max(padding, window.innerWidth - width - padding);
      const left = clamp(x, padding, maxLeft);

      const spaceAbove = y - padding;
      const spaceBelow = window.innerHeight - y - padding;
      const preferUp = preferVertical === 'up' || (preferVertical === 'auto' && y > window.innerHeight / 2);

      let top;
      if (preferUp) {
          if (spaceAbove >= height) top = y - height;
          else if (spaceBelow >= height) top = y;
      } else {
          if (spaceBelow >= height) top = y;
          else if (spaceAbove >= height) top = y - height;
      }

      if (!Number.isFinite(top)) {
          const maxTop = Math.max(padding, window.innerHeight - height - padding);
          top = clamp(y - Math.round(height / 2), padding, maxTop);
      }
  ...
  const handleWebviewContextMenu = (params) => {
      ...
      const openSubmenu = (triggerItem, entries) => {
          ...
          const submenuRect = measureMenuRect(submenu);
          ...
          if (spaceRight < submenuRect.width && spaceLeft > spaceRight) {
              x = rect.left - submenuRect.width - gap;
          }

          const anchorY = rect.top + Math.floor(rect.height / 2);
          placeMenuAtPoint(submenu, x, anchorY, {
              padding: viewportPadding,
              preferVertical: anchorY > window.innerHeight / 2 ? 'up' : 'down'
          });
      ...
      const systemEntries = [
          {
              ...
              click: () => window.browserAPI?.window?.minimize?.()
          },
          {
              type: 'action',
              label: 'Maximize / Restore',
              icon: 'M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14z',
              click: () => window.browserAPI?.window?.toggleMaximize?.()
          },
          {
              type: 'action',
              label: 'Close',
              icon: 'M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z',
              click: () => window.browserAPI?.window?.close?.()
          }
      ];
      ...
      if (params.selectionText) {
          ...
          grammarEntries.push(
              { type: 'action', label: 'Full Check (Offline)', icon: 'M4 4h16v3H4V4zm0 6h16v3H4v-3zm0 6h10v3H4v-3z', click: () => grammarUI.show(params.selectionText, params.selectionX || params.x, params.selectionY || params.y, 'both') },
              { type: 'action', label: 'Spelling Check (Offline)', icon: 'M12 2l4 8h6l-5 4 2 8-7-5-7 5 2-8-5-4h6z', click: () => grammarUI.show(params.selectionText, params.selectionX || params.x, params.selectionY || params.y, 'spelling') },
              { type: 'action', label: 'Grammar Check (Offline)', icon: 'M3 5h18v2H3zm0 6h12v2H3zm0 6h8v2H3z', click: () => grammarUI.show(params.selectionText, params.selectionX || params.x, params.selectionY || params.y, 'grammar') },
              { type: 'action', label: 'Translate EN -> BN (Offline)', icon: 'M12.87 15.07l-2.54-2.51.03-.03c1.74-1.94 2.98-4.17 3.71-6.53H17V4h-7V2H8v2H1v2h11.17c-.71 2.06-1.75 4.05-3.04 5.81-1.01-1.12-1.87-2.34-2.57-3.64H4.5c.81 1.63 1.83 3.17 3.03 4.58L3.06 18.06l1.41 1.41 4.7-4.7 3.13 3.13 1.98-1.83z', click: () => grammarTranslateUI.show(params.selectionText, params.selectionX || params.x, params.selectionY || params.y) }
          );
      ...
      addSubmenu('Grammar', 'M4 4h16v3H4V4zm0 6h16v3H4v-3zm0 6h10v3H4v-3z', grammarEntries);
      ...
      placeMenuAtPoint(webviewContextMenu, params.x, params.y, {
          padding: 8,
          preferVertical: params.y > window.innerHeight / 2 ? 'up' : 'down'
      });
  ...
  const handleTabContextMenu = async (id, x, y) => {
      ...
      placeMenuAtPoint(tabContextMenu, x, y, {
          padding: 8,
          preferVertical: y > window.innerHeight / 2 ? 'up' : 'down'
      });
      showContextMenuBackdrop();
  ...
  const handleSidePanelContextMenu = (x, y) => {
      ...
      placeMenuAtPoint(sidePanelContextMenu, x, y, {
          padding: 8,
          preferVertical: y > window.innerHeight / 2 ? 'up' : 'down'
      });
      showContextMenuBackdrop();
  ...
  const handleHiddenSidebarContextMenu = (x, y) => {
      ...
      placeMenuAtPoint(hiddenSidebarContextMenu, x, y, {
          padding: 8,
          preferVertical: y > window.innerHeight / 2 ? 'up' : 'down'
      });
      showContextMenuBackdrop();
  ...
  const btnMin = document.getElementById('chrome-win-min') || document.getElementById('btn-min');
  const btnMax = document.getElementById('chrome-win-max') || document.getElementById('btn-max');
  const btnClose = document.getElementById('chrome-win-close') || document.getElementById('btn-close');
  ...
  const getYouTubeAddonSettings = (settings = cachedSettings) => {
      ...
      return {
          ...
          cleanUi: adBlocker.ytCleanUi === true,
  ...
  const setupYouTubeAddonEvents = () => {
      ...
      youtubeAddonToggleCleanUi?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ ytCleanUi: youtubeAddonToggleCleanUi.checked });
      });
  ...
  // Now initialize the UI components after TabManager is created
    ...
    grammarUI = new GrammarUI(tabManager);
    grammarTranslateUI = new GrammarTranslateUI(tabManager);
  ...
  if (window.browserAPI.onShortcut) {
      window.browserAPI.onShortcut((command) => {
          ...
          else if (command === 'toggle-bookmarks') { openAppTab(BOOKMARKS_URL); }
          else if (command === 'toggle-electron-apps') { openAppTab(MINI_APPS_URL); }
          else if (command === 'toggle-extensions') { openAppTab(EXTENSIONS_URL); }
      });
  }
  ...
  if (btnNavDownloads) btnNavDownloads.onclick = () => openAppTab(DOWNLOADS_URL);
  ...
  // Expose chrome menu actions from the main renderer scope.
  window.chromeMenuBridge = {
      newTab: () => searchSystem.handleNewTabRequest(),
      openHistory: () => openAppTab(HISTORY_URL),
      openDownloads: () => openAppTab(DOWNLOADS_URL),
      openBookmarks: () => openAppTab(BOOKMARKS_URL),
      openMiniApps: () => openAppTab(MINI_APPS_URL),
      openExtensions: () => openAppTab(EXTENSIONS_URL),
      openAIChat: () => openAppTab(AI_CHAT_URL),
      openNeuralHub: () => openAppTab(NEURAL_HUB_URL),
      openGames: () => openAppTab(GAMES_URL),
      openMinecraft: () => openAppTab(MINECRAFT_URL),
      openServerOperator: () => openAppTab(SERVER_OPERATOR_URL),
      openSettings: () => openAppTab(SETTINGS_URL)
  };
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      if (grammarUI) grammarUI.hide();
      if (grammarTranslateUI) grammarTranslateUI.hide();
      closeAllContextMenus();
    }
  });
  ...
  const makeThemeAwareTabIcon = (svgMarkup) => {
      const style = getComputedStyle(document.body);
      const fg = (style.getPropertyValue('--text-main') || '#ffffff').trim();
      const bg = (style.getPropertyValue('--accent-color') || '#6366f1').trim();
      const inner = String(svgMarkup || '')
        .replace(/^[\s\S]*?<svg[^>]*>/i, '')
        .replace(/<\/svg>\s*$/i, '')
        .replace(/currentColor/gi, fg);
      const wrapped = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <rect x="1.5" y="1.5" width="21" height="21" rx="6" fill="${bg}"/>
        <g transform="translate(4 4) scale(0.6667)">${inner}</g>
      </svg>`;
      ...
      PRESET_ICONS.forEach(icon => {
          ...
          opt.onclick = () => {
              if (currentTabContextId) {
                  const dataUri = makeThemeAwareTabIcon(icon.svg);
  ...
  document.getElementById('ctx-trust-site').onclick = async () => {
    ...
    if (tab) {
        ...
        if (success) {
            ...
            // Notify system panel to refresh safe sites list
            window.dispatchEvent(new CustomEvent('site-trusted', { detail: { url: tab.url } }));
        }
    }
  };
...
// Chrome-toolbar bridge (chrome UI only): map chrome controls to default actions.
document.addEventListener('DOMContentLoaded', () => {
  const chromeBack = document.getElementById('chrome-btn-back');
  const chromeForward = document.getElementById('chrome-btn-forward');
  const chromeReload = document.getElementById('chrome-btn-reload');
  const chromeHome = document.getElementById('chrome-btn-home');
  const chromeSettings = document.getElementById('chrome-btn-settings');
  const chromeMenuBtn = document.getElementById('chrome-btn-menu');
  const chromeMenuPopup = document.getElementById('chrome-menu-popup');
  const chromeMenuZoomValue = document.getElementById('chrome-menu-zoom-value');
  const chromeMenuEyeValue = document.getElementById('chrome-menu-eye-value');
  const chromeAddress = document.getElementById('chrome-address-input');
  const defaultHome = document.getElementById('btn-nav-home');
  const defaultSettings = document.getElementById('btn-nav-settings');
  const defaultNewTab = document.getElementById('btn-new-tab');
  const defaultHistory = document.getElementById('btn-nav-history');
  const defaultDownloads = document.getElementById('btn-nav-downloads');
  const overlayInput = document.getElementById('overlay-input');
  const searchOverlay = document.getElementById('search-overlay');
  const webviewContainer = document.getElementById('webview-container');

  const getActiveWebview = () => (
    document.querySelector('#webview-container webview:not(.hidden)')
    || document.querySelector('#webview-container webview')
  );
  const HOME_URL = new URL('../../html/pages/home.html', import.meta.url).href;
  const getBridge = () => window.chromeMenuBridge || {};
  const MIN_ZOOM_FACTOR = 0.5;
  const MAX_ZOOM_FACTOR = 2.0;
  const ZOOM_STEP = 0.05;
  const MIN_EYE_LEVEL = 0;
  const MAX_EYE_LEVEL = 100;
  const EYE_STEP = 10;
  const EYE_STORAGE_KEY = 'chrome_eye_protection_level';
  let eyeProtectionLevel = Math.max(
    MIN_EYE_LEVEL,
    Math.min(MAX_EYE_LEVEL, Number(localStorage.getItem(EYE_STORAGE_KEY)) || 0)
  );
  ...
  chromeHome?.addEventListener('click', () => {
    trigger(defaultHome);
    // Fallback: force active/new tab to Home if default handler didn't resolve it.
    setTimeout(() => {
      let active = document.querySelector('#webview-container webview:not(.hidden)');
      if (active) {
        const current = String(active.getURL?.() || active.src || '');
        if (!current.includes('pages/home.html')) {
          try { active.src = HOME_URL; } catch (_) {}
        }
        return;
      }
      trigger(defaultNewTab);
      setTimeout(() => {
        active = document.querySelector('#webview-container webview:not(.hidden)');
        if (active) {
          try { active.src = HOME_URL; } catch (_) {}
        }
      }, 80);
  ...
  chromeSettings?.addEventListener('click', () => trigger(defaultSettings));
  ...
  const setAddressValue = () => {
    if (!chromeAddress || document.activeElement === chromeAddress) return;
    const wv = getActiveWebview();
    const currentUrl = String(wv?.getURL?.() || wv?.src || '').trim();
    if (!currentUrl) {
      chromeAddress.value = '';
      return;
    }
    if (isInternalOmxPage(currentUrl)) {
      chromeAddress.value = '';
      return;
    }
    chromeAddress.value = currentUrl;
  };

  const isInternalOmxPage = (value = '') => {
    const raw = String(value || '').trim();
    if (!raw) return false;
    try {
      const parsed = new URL(raw, window.location.href);
      const p = String(parsed.pathname || '').replace(/\\/g, '/').toLowerCase();
      return p.includes('/html/pages/') || p.includes('/html/windows/');
    } catch (_) {
      return raw.includes('html/pages/') || raw.includes('html/windows/');
    }
  };
  ...
  const looksLikeUrl = (value = '') => {
    ...
    if (v.startsWith('localhost') || /^\d{1,3}(\.\d{1,3}){3}(:\d+)?(\/|$)/.test(v)) return true;
    return /^[^\s]+\.[^\s]{2,}(\/.*)?$/.test(v);
  };
  ...
  const navigateFromAddressBar = (value = '') => {
    ...
    setTimeout(() => {
      ...
      if (active) {
        try { active.src = target; } catch (_) {}
      }
    }, 60);
  };

  const bindWebviewAddressSync = (webviewEl) => {
    if (!webviewEl || webviewEl.dataset.chromeAddressBound === '1') return;
    webviewEl.dataset.chromeAddressBound = '1';
    const sync = () => setTimeout(setAddressValue, 0);
    webviewEl.addEventListener('did-navigate', sync);
    webviewEl.addEventListener('did-navigate-in-page', sync);
    webviewEl.addEventListener('did-stop-loading', sync);
  };

  const buildEyeProtectionFilter = (level) => {
    const safeLevel = Math.max(MIN_EYE_LEVEL, Math.min(MAX_EYE_LEVEL, Number(level) || 0));
    if (safeLevel <= 0) return '';
    const t = safeLevel / 100;
    // Amber/yellow "night light" style filter profile.
    const sepia = (25 + t * 75).toFixed(1);
    const hueRotate = (-12 - t * 10).toFixed(1);
    const saturate = (1.05 + t * 0.2).toFixed(3);
    const brightness = (1 - t * 0.09).toFixed(3);
    const contrast = (1 - t * 0.08).toFixed(3);
    return `sepia(${sepia}%) hue-rotate(${hueRotate}deg) saturate(${saturate}) brightness(${brightness}) contrast(${contrast})`;
  };

  const applyEyeProtectionToWebview = (wv) => {
    if (!wv) return;
    const filter = buildEyeProtectionFilter(eyeProtectionLevel);
    try {
      wv.style.filter = filter;
      wv.style.transition = 'filter 140ms ease';
    } catch (_) {}
  };

  const applyEyeProtectionToAllWebviews = () => {
    document.querySelectorAll('#webview-container webview').forEach(applyEyeProtectionToWebview);
  };

  const updateChromeEyeButtonState = (level) => {
    const eyeOutBtn = chromeMenuPopup?.querySelector('[data-menu-action="eye-out"]');
    const eyeInBtn = chromeMenuPopup?.querySelector('[data-menu-action="eye-in"]');
    if (eyeOutBtn) {
      const disabled = level <= MIN_EYE_LEVEL;
      eyeOutBtn.disabled = disabled;
      eyeOutBtn.setAttribute('aria-disabled', disabled ? 'true' : 'false');
    }
    if (eyeInBtn) {
      const disabled = level >= MAX_EYE_LEVEL;
      eyeInBtn.disabled = disabled;
      eyeInBtn.setAttribute('aria-disabled', disabled ? 'true' : 'false');
    }
  };

  const syncChromeEyeLabel = () => {
    if (!chromeMenuEyeValue) return;
    const level = Math.max(MIN_EYE_LEVEL, Math.min(MAX_EYE_LEVEL, Number(eyeProtectionLevel) || 0));
    chromeMenuEyeValue.textContent = `${level}%`;
    updateChromeEyeButtonState(level);
  };

  const setEyeProtectionLevel = (level) => {
    eyeProtectionLevel = Math.max(MIN_EYE_LEVEL, Math.min(MAX_EYE_LEVEL, Number(level) || 0));
    try { localStorage.setItem(EYE_STORAGE_KEY, String(eyeProtectionLevel)); } catch (_) {}
    applyEyeProtectionToAllWebviews();
    syncChromeEyeLabel();
  };

  const adjustEyeProtection = (delta) => {
    setEyeProtectionLevel((Number(eyeProtectionLevel) || 0) + (Number(delta) || 0));
  };

  if (webviewContainer) {
    webviewContainer.querySelectorAll('webview').forEach(bindWebviewAddressSync);
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node?.tagName?.toLowerCase() === 'webview') {
            bindWebviewAddressSync(node);
            applyEyeProtectionToWebview(node);
          }
        });
      });
      setAddressValue();
    });
    observer.observe(webviewContainer, { childList: true });
  }

  const setChromeMenuOpen = (open) => {
    if (!chromeMenuBtn || !chromeMenuPopup) return;
    chromeMenuPopup.classList.toggle('hidden', !open);
  ...
  const getZoomFactor = async (wv) => {
    if (!wv || typeof wv.getZoomFactor !== 'function') return 1;

    // Newer implementations can return a Promise/number directly.
    try {
      const result = wv.getZoomFactor();
    ...
    // Older implementations use callback style.
    try {
      const value = await new Promise((resolve) => {
        ...
        try {
          wv.getZoomFactor((factor) => done(factor));
          setTimeout(() => done(1), 150);
        } catch (_) {
          done(1);
        }
      });
  ...
  const setZoomFactor = (wv, factor) => {
    if (!wv || typeof wv.setZoomFactor !== 'function') return;
    const clamped = Math.max(MIN_ZOOM_FACTOR, Math.min(MAX_ZOOM_FACTOR, Number(factor) || 1));
    try {
      wv.setZoomFactor(clamped);
    } catch (_) {
      try { wv.setZoomFactor(clamped, () => {}); } catch (_) {}
    }
  };

  const updateChromeZoomButtonState = (factor) => {
    const zoomOutBtn = chromeMenuPopup?.querySelector('[data-menu-action="zoom-out"]');
    const zoomInBtn = chromeMenuPopup?.querySelector('[data-menu-action="zoom-in"]');
    if (zoomOutBtn) {
      const disabled = factor <= MIN_ZOOM_FACTOR + 0.001;
      zoomOutBtn.disabled = disabled;
      zoomOutBtn.setAttribute('aria-disabled', disabled ? 'true' : 'false');
    }
    if (zoomInBtn) {
      const disabled = factor >= MAX_ZOOM_FACTOR - 0.001;
      zoomInBtn.disabled = disabled;
      zoomInBtn.setAttribute('aria-disabled', disabled ? 'true' : 'false');
    }
  };

  const syncChromeZoomLabel = async () => {
    if (!chromeMenuZoomValue) return;
    const wv = getActiveWebview();
    const factor = await getZoomFactor(wv);
    const percentage = factor * 100;
    chromeMenuZoomValue.textContent = Number.isInteger(percentage)
      ? `${percentage}%`
      : `${percentage.toFixed(1)}%`;
    updateChromeZoomButtonState(factor);
  };

  const adjustChromeZoom = async (delta) => {
    const wv = getActiveWebview();
    if (!wv) return;
    const factor = await getZoomFactor(wv);
    setZoomFactor(wv, factor + delta);
    setTimeout(syncChromeZoomLabel, 30);
  };

  chromeMenuBtn?.addEventListener('click', (event) => {
    event.stopPropagation();
    const isOpen = !chromeMenuPopup?.classList.contains('hidden');
    setChromeMenuOpen(!isOpen);
  });

  chromeMenuPopup?.addEventListener('click', (event) => {
    const item = event.target.closest('.chrome-menu-item');
    const zoomBtn = event.target.closest('.chrome-zoom-btn');
    const action = item?.dataset.menuAction || zoomBtn?.dataset.menuAction;
    if (!action) return;

    const bridge = getBridge();
    if (action === 'new-tab') bridge.newTab?.() || trigger(defaultNewTab);
    else if (action === 'history') bridge.openHistory?.() || trigger(defaultHistory);
    else if (action === 'downloads') bridge.openDownloads?.() || trigger(defaultDownloads);
    else if (action === 'bookmarks') bridge.openBookmarks?.();
    else if (action === 'mini-apps') bridge.openMiniApps?.();
    else if (action === 'extensions') bridge.openExtensions?.();
    else if (action === 'ai-chat') bridge.openAIChat?.();
    else if (action === 'neural-hub') bridge.openNeuralHub?.();
    else if (action === 'games') bridge.openGames?.();
    else if (action === 'minecraft') bridge.openMinecraft?.();
    else if (action === 'server-operator') bridge.openServerOperator?.();
    else if (action === 'settings') bridge.openSettings?.() || trigger(defaultSettings);
    else if (action === 'zoom-out') adjustChromeZoom(-ZOOM_STEP);
    else if (action === 'zoom-in') adjustChromeZoom(ZOOM_STEP);
    else if (action === 'zoom-reset') {
      const wv = getActiveWebview();
      setZoomFactor(wv, 1);
      setTimeout(syncChromeZoomLabel, 30);
    }
    else if (action === 'eye-out') adjustEyeProtection(-EYE_STEP);
    else if (action === 'eye-in') adjustEyeProtection(EYE_STEP);
    else if (action === 'eye-reset') setEyeProtectionLevel(0);
    if (!action.startsWith('zoom-') && !action.startsWith('eye-')) setChromeMenuOpen(false);
  });

  document.addEventListener('mousedown', (event) => {
    if (!chromeMenuBtn || !chromeMenuPopup) return;
    if (chromeMenuPopup.classList.contains('hidden')) return;
    if (chromeMenuPopup.contains(event.target) || chromeMenuBtn.contains(event.target)) return;
    setChromeMenuOpen(false);
  });

  // Webview interactions may not bubble through normal document listeners.
  webviewContainer?.addEventListener('mousedown', () => {
    if (!chromeMenuPopup || chromeMenuPopup.classList.contains('hidden')) return;
    setChromeMenuOpen(false);
  });
  ...
  // Apply persisted eye protection immediately on load.
  applyEyeProtectionToAllWebviews();
  syncChromeEyeLabel();

  // Real address bar behavior: show URL, allow copy/edit, navigate on Enter.
  const isHomeTabActive = () => {
    const wv = getActiveWebview();
    const current = String(wv?.getURL?.() || wv?.src || '').trim();
    return current === HOME_URL || current.includes('pages/home.html');
  };

  const openShortcutSearch = (seed = '', submit = false) => {
    trigger(defaultNewTab);
    setTimeout(() => {
      if (!overlayInput) return;
      overlayInput.value = String(seed || '');
      ...
      if (submit) {
        overlayInput.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true, cancelable: true }));
      }
    }, 40);
  };

  chromeAddress?.addEventListener('click', (event) => {
    if (!isHomeTabActive()) return;
    event.preventDefault();
    const visible = !searchOverlay?.classList?.contains('hidden');
    if (!visible) openShortcutSearch(chromeAddress.value, false);
    chromeAddress.blur();
  });

  chromeAddress?.addEventListener('focus', () => {
    if (isHomeTabActive()) {
      const visible = !searchOverlay?.classList?.contains('hidden');
      if (!visible) openShortcutSearch(chromeAddress.value, false);
      chromeAddress.blur();
    ...
    chromeAddress.select();
  });
  chromeAddress?.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    if (isHomeTabActive()) {
      openShortcutSearch(chromeAddress.value, true);
      return;
    }
    navigateFromAddressBar(chromeAddress.value);
  });

  document.addEventListener('click', (event) => {
    if (event.target?.closest('.tab-item')) {
      setTimeout(setAddressValue, 0);
    }
  });
  window.addEventListener('focus', setAddressValue);
  setTimeout(setAddressValue, 40);
});
