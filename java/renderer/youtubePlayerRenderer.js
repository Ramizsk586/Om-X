document.addEventListener('DOMContentLoaded', async () => {
    ...
    const isValidYouTubeId = (value = '') => /^[A-Za-z0-9_-]{11}$/.test(String(value));
    ...
    const createVideoCard = (video) => {
        ...
        const safeTitle = escapeHtml(video.title || 'YouTube Video');
        card.innerHTML = `
    ...
    const performSearch = async () => {
        ...
        try {
            ...
            els.grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:100px; color:#ef4444;">Neural Engine Timeout: ${escapeHtml(e?.message || 'Unknown error')}</div>`;
        } finally {
    ...
    const renderGrid = (videos) => {
        ...
        videos.filter(v => v.url.includes('youtube.com') || v.url.includes('youtu.be')).forEach(v => {
            ...
            const safeTitle = escapeHtml(v.title || 'YouTube Video');
    ...
    const sendChat = async () => {
        ...
        try {
            ...
            loadingDiv.textContent = `Error: ${String(e?.message || 'Unknown error')}`;
        }
    };
