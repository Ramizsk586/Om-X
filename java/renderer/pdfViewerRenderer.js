...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderProfileCards = () => {
        ...
        profiles.forEach((profile) => {
            ...
            const safeProvider = escapeHtml(String(profile.provider || 'unknown'));
            const safeModel = escapeHtml(String(profile.model || 'default-model'));
            const safeBaseUrl = escapeHtml(String(profile.baseUrl || ''));
            card.innerHTML = `
                <div class="profile-picker-provider">${safeProvider}</div>
                <div class="profile-picker-model">${safeModel}</div>
                <div class="profile-picker-meta">
                    ${safeBaseUrl ? `<div>URL: ${safeBaseUrl}</div>` : ''}
                    <div>Key: ${escapeHtml(keyMasked)}</div>
                </div>
            `;
    ...
    const performAiCommand = async (prompt, system, imageData = null) => {
        ...
        try {
            ...
            responseMsg.textContent = `Error: ${String(e?.message || 'Unknown error')}`;
