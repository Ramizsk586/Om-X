document.addEventListener('DOMContentLoaded', async () => {
    const els = {
        ...
        btnInstallEngine: document.getElementById('btn-install-engine'),
        engineNotInstalled: document.getElementById('engine-not-installed'),
        engineDownloading: document.getElementById('engine-downloading'),
        engineExtracting: document.getElementById('engine-extracting'),
        engineInstalled: document.getElementById('engine-installed'),
        engineStatusPill: document.getElementById('engine-global-status'),
        engineProg: document.getElementById('engine-progress-fill')
    };
    ...
    const updateEngineUI = (status) => {
        els.engineNotInstalled.classList.toggle('hidden', status !== 'not_installed');
        els.engineDownloading.classList.toggle('hidden', status !== 'downloading');
        els.engineExtracting.classList.toggle('hidden', status !== 'extracting');
        els.engineInstalled.classList.toggle('hidden', status !== 'installed');
        
        if (status === 'installed') {
            els.engineStatusPill.textContent = 'ENGINE: READY';
            els.engineStatusPill.classList.add('active');
        } else {
            els.engineStatusPill.textContent = `ENGINE: ${status.toUpperCase().replace('_',' ')}`;
            els.engineStatusPill.classList.remove('active');
        }
    };
    ...
    const renderPresets = (data) => {
        ...
        data.presets.forEach(preset => {
            ...
            item.innerHTML = `
                ...
                ${isDownloaded && !isActive ? `<button class="btn-action btn-load" data-id="${preset.id}" ${data.engineStatus !== 'installed' ? 'disabled' : ''}>${data.engineStatus === 'installed' ? 'Mount to GPU' : 'Wait for Engine'}</button>` : ''}
