...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Attach to static shortcut inputs
  shortcutInputs.forEach(attachShortcutRecorder);
  
  // Attach to Add Engine trigger input
  attachShortcutRecorder(seTriggerInput);

  // Helper: Populate Models
  ...
  function renderEngines() {
     ...
     document.querySelectorAll('.delete-shortcut-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
           ...
           if (engine.id === currentSettings.defaultSearchEngineId) {
               alert("Cannot delete the active default search engine.");
               return;
           }
     ...
     document.querySelectorAll('.engine-name-input').forEach(input => {
        input.addEventListener('input', (e) => {
           const idx = parseInt(e.target.dataset.index);
           currentSettings.searchEngines[idx].name = e.target.value;
        });
     });

     document.querySelectorAll('.engine-trigger-input').forEach(input => {
        attachShortcutRecorder(input);
        input.addEventListener('input', (e) => {
           const idx = parseInt(e.target.dataset.index);
           currentSettings.searchEngines[idx].trigger = e.target.value;
        });
     });
  }

  // Helper: Render Search Cards
  function renderSearchCards() {
      ...
      document.querySelectorAll('.card-toggle').forEach(btn => {
          btn.addEventListener('click', (e) => {
              ...
              currentSettings.defaultSearchEngineId = id;
              renderSearchCards();
          });
      });
  }
  ...
  function syncUI() {
    ...
    if (currentSettings.features) {
       ...
       toggleHistory.checked = currentSettings.features.enableHistory !== false; // New
    ...
    ENGINE_PRESETS.forEach(preset => {
        if (!currentSettings.searchEngines.find(e => e.id === preset.id)) {
            currentSettings.searchEngines.push(preset);
        }
    });
  ...
  try {
    ...
    if (saved && typeof saved === 'object') {
      ...
      if (!currentSettings.features) currentSettings.features = { enableAdBlocker: true, enableSafeSearch: true, enableScreenshot: true, enableTextStudio: true, enableHistory: true };
    ...
    if (name && url) {
       if (!icon) {
          try {
             let domain = url;
             if (url.includes('://')) {
                domain = new URL(url.replace('%s', '')).hostname;
             } else {
                const parts = url.split('/');
                if (parts[0].includes('.')) domain = parts[0];
             }
             icon = `https://www.google.com/s2/favicons?domain=${domain}&sz=64`;
          } catch (e) { icon = ''; }
       }

       currentSettings.searchEngines.push({
          id: Date.now().toString(),
          name, trigger, url, icon
       });
       
       seNameInput.value = '';
       seTriggerInput.value = '';
       seUrlInput.value = '';
       seIconInput.value = '';
  ...
  providerSelect.addEventListener('change', () => {
    ...
    if (currentSettings.providers && currentSettings.providers[oldProvider]) {
      currentSettings.providers[oldProvider].key = apiKeyInput.value;
      currentSettings.providers[oldProvider].model = modelSelect.value;
    }
  ...
  async function loadUpdateHistory() {
    ...
    if (history.length === 0) {
      // Create a fake current entry if list is empty
      versionListEl.innerHTML = `
        <div class="version-item current">
           <div class="version-dot"></div>
           <div class="version-info">
              <div class="version-number">Current Version</div>
              <div class="version-date">Initial Install</div>
           </div>
        </div>
      `;
      return;
    ...
    history.forEach(item => {
       ...
       let actionBtn = '';
       if (!item.isCurrent) {
          // Only show rollback if we have the installer cached
          if (item.installerExists) {
             actionBtn = `<button class="btn-rollback" data-version="${item.version}">Rollback</button>`;
          } else {
             actionBtn = `<button class="btn-rollback" disabled title="Installer not found">Unavailable</button>`;
          }
       }
       ...
       el.innerHTML = `
          <div class="version-dot"></div>
          <div class="version-info">
             <div class="version-number">v${item.version} ${item.isCurrent ? '(Current)' : ''}</div>
             <div class="version-date">Installed: ${date}</div>
          </div>
    ...
    document.querySelectorAll('.btn-rollback').forEach(btn => {
       if(!btn.disabled) {
          btn.addEventListener('click', async () => {
             ...
             if (confirm(`Rollback to version ${ver}? The application will restart.`)) {
                const res = await window.browserAPI.updater.rollback(ver);
                if (!res.success) alert("Rollback failed: " + res.msg);
             }
          });
       }
    });
  ...
  if (window.browserAPI.updater) {
     window.browserAPI.updater.onStatus((status) => {
        if (status.status === 'checking') {
           updateMsg.textContent = status.msg;
           ...
           updateBar.style.width = status.percent + '%';
           btnDownloadUpdate.style.display = 'none';
        } else if (status.status === 'downloaded') {
           updateMsg.textContent = 'Ready to Install';
           updateProgress.style.display = 'none';
           btnRestartUpdate.style.display = 'inline-block';
        } else if (status.status === 'uptodate') {
           updateMsg.textContent = status.msg;
           setTimeout(() => { updateMsg.textContent = 'System is up to date.'; }, 3000);
        } else if (status.status === 'error') {
           updateMsg.textContent = 'Error: ' + status.msg;
        }
     });
  ...
  // --- Build & Verify Panel Handlers ---

  if (btnVerifyApi) {
    btnVerifyApi.addEventListener('click', async () => {
        // Use separate Lab Key
        const key = labApiKeyInput.value.trim();
        const model = 'gemini-2.5-flash';
        
        if (!key) {
            apiVerifyResult.textContent = 'Error: Please enter a Lab API Key.';
            apiVerifyResult.style.color = '#ff5252';
            return;
        }

        apiVerifyResult.textContent = 'Connecting...';
        apiVerifyResult.style.color = '#aaa';
        ...
        try {
            ...
            if (response.text) {
                apiVerifyResult.textContent = `Success! Response: "${response.text.trim()}" (${duration}ms)`;
                apiVerifyResult.style.color = '#00e676';
            }
        } catch(e) {
            apiVerifyResult.textContent = 'Connection Failed: ' + e.message;
            apiVerifyResult.style.color = '#ff5252';
        }
    });
  }
  ...
  if (btnTestClassifier) {
      btnTestClassifier.addEventListener('click', async () => {
          ...
          if (!url) return;
          if (!key) {
              alert("Please enter a Lab API Key in the Lab Environment section.");
              return;
          }

          classifierResult.style.display = 'block';
          clfStatus.textContent = 'ANALYZING...';
          clfStatus.style.color = '#aaa';
          clfReason.textContent = '';
          ...
          try {
            ...
            clfStatus.textContent = result;
            clfStatus.style.color = result.includes('SAFE') ? '#00e676' : '#ff5252';
            clfReason.textContent = `Latency: ${duration}ms\nRaw Output: ${result}`;

          } catch(e) {
              clfStatus.textContent = 'ERROR';
              clfStatus.style.color = '#ff5252';
              clfReason.textContent = e.message;
          }
      });
  }
  ...
  btnSave.addEventListener('click', async () => {
    try {
      ...
      currentSettings.aiConfig = {
        enablePageVision: toggleVision.checked,
        enableWebScraping: toggleScraping.checked,
        enableBrowserControl: toggleControl.checked
      };
      ...
      if (success) {
        statusEl.textContent = 'System configuration updated.';
