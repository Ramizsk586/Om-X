...
class LlamaServerRenderer {
    ...
    showPathPrompt(type) {
        ...
        const path = prompt(`Enter the path to ${type === 'llama' ? 'llama-server.exe' : 'models folder'}:`, placeholder);
        if (path) {
            if (type === 'llama') {
                const pathInput = document.getElementById('llama-path-input');
                if (pathInput) {
    ...
    async scanModelsFolder() {
        ...
        this.log(`Scanning models folder: ${path}`, 'info');

        if (window.browserAPI && window.browserAPI.files) {
            try {
                const files = await window.browserAPI.files.readDir(path);
