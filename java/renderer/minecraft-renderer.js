...
class LlamaServerRenderer {
    ...
    setupEventListeners() {
        ...
        if (commandInput) {
            commandInput.addEventListener('keydown', (e) => {
                ...
                if (!this.isRunning) {
                    this.log('Server is offline. Start Llama Server first.', 'warning');
                    return;
                }
    ...
    updateServerInfo() {
        ...
        const portEl = document.getElementById('bedrock-port');
        if (playersEl) playersEl.textContent = `${this.playerCount} / ${this.maxPlayers}`;
        if (portEl) portEl.textContent = this.currentPort || '--';
    }

    parseBedrockOutput(line) {
        if (!line) return;
        const text = String(line);

        const portMatch = text.match(/port:\s*(\d{2,5})/i);
        if (portMatch && portMatch[1]) {
            this.currentPort = portMatch[1];
        }

        const fractionMatch =
            text.match(/(\d+)\s*\/\s*(\d+)\s*(?:players?|online)/i) ||
            text.match(/players?\s*[:=]\s*(\d+)\s*\/\s*(\d+)/i) ||
            text.match(/there are\s*(\d+)\s*\/\s*(\d+)\s*players?\s*online/i);

        if (fractionMatch) {
            const p = parseInt(fractionMatch[1], 10);
            const m = parseInt(fractionMatch[2], 10);
            if (!Number.isNaN(p)) this.playerCount = Math.max(0, p);
            if (!Number.isNaN(m) && m > 0) this.maxPlayers = m;
        } else {
            if (/player connected|joined the game/i.test(text)) {
                this.playerCount = Math.min(this.playerCount + 1, this.maxPlayers);
            }
            if (/player disconnected|left the game/i.test(text)) {
...
console.log('[ServerRenderer] Initialization complete');
    ...
    setupEventListeners() {
        ...
        // Cloud provider cards - handle link clicks robustly
        ...
        cloudProviderCards.forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                try {
                    if (window.browserAPI?.openTab && typeof window.browserAPI.openTab === 'function') {
                        console.log('[ServerRenderer] Opening cloud provider via browserAPI.openTab:', url);
                        window.browserAPI.openTab(url);
                        return;
                    }
                } catch (err) {
                    console.error('[ServerRenderer] openTab failed, falling back to window.open:', err);
                }
        ...
        if (commandInput) {
            commandInput.addEventListener('keydown', (e) => {
                ...
                if (!this.isBedrockRunning) {
                    this.log('Bedrock server is offline. Start the server first.', 'warning');
                    return;
                }
...
class AIPlayerRenderer {
    ...
    applyAiWindowMode() {
        ...
        this.ensureAiWindowControls();
        this.setupAiWindowContextMenu();
    }

    ensureAiWindowControls() {
        const header = document.querySelector('.header');
        if (!header) return;
        if (document.getElementById('ai-window-controls')) return;

        const controls = document.createElement('div');
        controls.id = 'ai-window-controls';
        ...
        controls.innerHTML = `
            <button type="button" data-action="minimize" title="Minimize" style="width:34px;height:34px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text);cursor:pointer;">-</button>
            <button type="button" data-action="maximize" title="Maximize / Restore" style="width:34px;height:34px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text);cursor:pointer;">?</button>
            <button type="button" data-action="close" title="Close" style="width:34px;height:34px;border-radius:8px;border:1px solid var(--border);background:rgba(239,68,68,0.18);color:#fecaca;cursor:pointer;">�</button>
        `;

        const statusBadge = header.querySelector('.status-badge');
        if (statusBadge?.parentElement === header) {
            header.insertBefore(controls, statusBadge);
        } else {
...
class JavaServerRenderer {
    ...
    setupEventListeners() {
        ...
        // Tab switching
        const javaTabs = document.querySelectorAll('.java-tab');
        javaTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                const tabName = tab.dataset.javaTab;
                this.switchJavaTab(tabName);
            });
        });
        // Browse button
    ...
    switchJavaTab(tabName) {
        document.querySelectorAll('.java-tab').forEach(tab => {
            tab.classList.toggle('active', tab.dataset.javaTab === tabName);
        });

        document.querySelectorAll('.java-tab-content').forEach(content => {
            content.classList.toggle('active', content.id === `java-tab-${tabName}`);
        });
    }
