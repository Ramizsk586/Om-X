...
document.addEventListener('DOMContentLoaded', async () => {
    const els = {
        ...
        // eliza config elements
        elizaRandom: document.getElementById('eliza-random-toggle'),
        elizaEnhanced: document.getElementById('eliza-enhanced-toggle'),
        elizaMemSize: document.getElementById('eliza-mem-size'),
    ...
    const loadConfig = async () => {
        ...
        // eliza options
        if (els.elizaRandom) els.elizaRandom.checked = !(config.elizaNoRandom || false);
        if (els.elizaEnhanced) els.elizaEnhanced.checked = config.elizaEnhanced || false;
        if (els.elizaMemSize) els.elizaMemSize.value = config.elizaMemSize || 20;
    ...
    if (els.provider) {
        els.provider.onchange = async () => {
            ...
            if (els.apiKey) {
                const noKeyRequired = p === 'offline' || p === 'eliza' || p === 'ollama' || p === 'lmstudio' || p === 'llamacpp';
    ...
    if (els.btnSave) {
        els.btnSave.onclick = async () => {
            ...
            // persist Eliza-specific options
            config.elizaNoRandom = !(els.elizaRandom?.checked ?? true);
