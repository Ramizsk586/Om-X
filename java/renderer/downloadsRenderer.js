...
async function performAction(action, id) {
  ...
  try {
    if (action === 'open') {
      const result = await window.browserAPI.downloads.openFile(id);
      if (result?.handledInternally && result.url) {
        if (window.browserAPI?.openTab) window.browserAPI.openTab(result.url);
        else if (window.browserAPI?.navigate) window.browserAPI.navigate(result.url);
      }
    }
