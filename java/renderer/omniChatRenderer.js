...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!document.getElementById(styleId)) {
        ...
        style.textContent = `
            ...
            /* Keep only reasoning and code as visible containers */
            .collapsible-container:not(.reasoning):not(.code) {
                background: transparent !important;
                border: none !important;
                box-shadow: none !important;
                margin: 8px 0 !important;
            }
            .collapsible-container:not(.reasoning):not(.code) .container-header {
                display: none !important;
            }
            .collapsible-container:not(.reasoning):not(.code) .container-content {
                max-height: none !important;
                opacity: 1 !important;
                background: transparent !important;
            }
            .collapsible-container:not(.reasoning):not(.code) .container-content-inner {
                padding: 0 !important;
            }
...
${nextStep}`;
    ...
    const ttsController = {
        ...
        stop() {
            ...
            // Restore original HTML and clear all references
            if (this.activeBubble && this.originalHTML) {
                try {
                    this.activeBubble.innerHTML = this.originalHTML;
                } catch (e) {
                    console.warn('Failed to restore bubble HTML:', e);
                }
            }
            
            // Reset all state variables completely to prevent carryover
            ...
            this.fallbackHighlightInterval = null;
        ...
        async speakWithHighlight(bubble, text, uiCallback) {
            ...
            // FIXED: Extract text fresh from the DOM element to avoid using cached text from first response
            let extractedText = text;
            if (bubble && bubble.textContent) {
                // Extract from the actual DOM to ensure we're using the current response
                const responseContent = bubble.querySelector('.final-answer-instant') || bubble;
                extractedText = responseContent.textContent || text;
            }
                ...
                container.innerHTML = `
                    <div class="container-header">
                        <div class="container-header-title">
                            ...
                            <span class="container-header-text">Manuscript</span>
                        </div>
                        <button class="toggle-btn" aria-label="Toggle manuscript">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <polyline points="6 9 12 15 18 9"></polyline>
                            </svg>
                        </button>
                    ...
                    <div class="container-content">
                        <div class="container-content-inner canvas-body manuscript"></div>
                    </div>
                `;
                this.el.appendChild(container);
                scrollToBottom();
...
Tell me which finding you want expanded, and I will drill deeper into that source set.${causeLine}`;
    ...
    const sendMessage = async () => {
        ...
        if (shouldUseDatabase(text)) {
            ...
            try {
                // Use local response directly without AI rewriting to avoid delay
        ...
        const isExplicitGreeting = (() => {
            ...
            return /^(hi|hello|hey|thanks|thank you|good morning|good afternoon|good evening)[!.?]*$/i.test(t);
        })();
        const greetingMatch = (!asset && modeAtSend === 'nothing' && isExplicitGreeting)
            ? QuickBot.findMatch(text)
            : null;
        if (greetingMatch) {
            const workbench = new NeuralWorkbench(responseBubble, responseMsgDiv);
            workbench.updateTask('intent', 'active');
            ...
            chatHistory.push({ role: 'user', parts: [{ text }] });
            chatHistory.push({ role: 'model', parts: [{ text: greetingMatch }] });
            finishGen(); 
            return;
        }
        ...
        try {
            ...
            if (codeBlocks.length > 0) {
                for (const match of codeBlocks) {
                    ...
                    await workbench.animateCanvas('code', code, lang);
    ...
    function syncOfflineModeLock() {
        offlineModeLocked = ['offline','eliza'].includes(String(config?.provider || '').toLowerCase());
        const modeButtons = [
