...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const bookmarkFallbackIcon = new URL('../../assets/icons/app.ico', import.meta.url).href;
  ...
  const bookmarkTopClose = document.getElementById('bookmark-top-close');
  ...
  const bookmarkDraftIcon = document.getElementById('bookmark-draft-icon');
  const bookmarkDraftUrl = document.getElementById('bookmark-draft-url');
  ...
  const hideFeaturesHomePopup = () => {
    if (featuresHomePopup) featuresHomePopup.classList.add('hidden');
  };
  const sanitizeBookmarkIcon = (value) => {
    const raw = String(value || '').trim();
    if (!raw) return bookmarkFallbackIcon;
    try {
      const parsed = new URL(raw, window.location.href);
      return ['http:', 'https:', 'file:', 'data:'].includes(parsed.protocol) ? raw : bookmarkFallbackIcon;
    } catch (_) {
      return bookmarkFallbackIcon;
    }
  };
  ...
  siteInfoOverlay.innerHTML = `
      <div id="site-info-panel" style="
          ...
          color: rgba(255,255,255,0.94);
      ">
          <div style="display:flex; align-items:center; justify-content:center; margin-bottom:8px; padding:1px 2px 8px; border-bottom:1px solid rgba(255,255,255,0.08);">
              <div style="width:100%; text-align:center; font-size:11px; font-weight:700; letter-spacing:.12em; text-transform:uppercase; color:rgba(255,255,255,0.72);">Page Info</div>
          </div>
          <div id="site-info-grid" style="
              display: grid;
  ...
  const setSiteInfoVisible = (visible) => {
      siteInfoOverlay?.classList.toggle('hidden', !visible);
  };
  ...
  const renderSiteInfoRows = (rows = []) => {
      ...
      siteInfoGrid.innerHTML = rows.map((row) => {
          ...
          let tone = 'neutral';
          if (label.toLowerCase() === 'safety') {
              if (/^safe$/i.test(value)) tone = 'ok';
              else if (/^danger$/i.test(value) || /^unsafe$/i.test(value) || /failed/i.test(value)) tone = 'error';
              else if (/^suspicious$/i.test(value)) tone = 'warn';
              else if (/^checking/i.test(value)) tone = 'info';
          }
          return `
              <div class="site-info-label">${escapeHtml(label)}</div>
              <div class="site-info-value" data-tone="${escapeHtml(tone)}">${escapeHtml(value)}</div>
          `;
      }).join('');
  };
  ...
  const resolveSiteSafetyLabel = (result = null, rawUrl = '') => {
      ...
      if (result.success) {
          return String(result.safety || (result.safe ? 'Safe' : 'Unknown')).trim() || 'Unknown';
      }
      return String(result.safety || result.error || 'Check failed').trim() || 'Check failed';
  };

  const primeSiteSafetyScan = (rawUrl = '') => {
      const currentUrl = String(rawUrl || '').trim();
      if (!isHttpWebsiteUrl(currentUrl)) return;
      if (!window.browserAPI?.security?.primeSiteSafetyScan) return;
      window.browserAPI.security.primeSiteSafetyScan({
          url: currentUrl,
          recordVisit: true
      }).catch(() => {});
  };
  ...
  const refreshSiteInfoPopup = async () => {
      ...
      const siteSafetyPromise = (async () => {
          if (!isHttpWebsiteUrl(currentUrl)) return null;
          try {
              return await window.browserAPI?.security?.getSiteSafetyStatus?.({
                  url: currentUrl,
                  allowScan: true
              });
          } catch (_) {
      ...
      const siteSafety = await siteSafetyPromise;
    ...
    (() => {
      try {
        ...
        const clickDarkControls = () => {
          const nodes = Array.from(document.querySelectorAll('button, a, label, [role="button"], input[type="radio"], input[type="checkbox"]'));
          for (const node of nodes) {
            ...
            if (!text) continue;
            const looksDarkOption = /(dark|night)/i.test(text);
            const looksThemeControl = /(theme|appearance|mode|dark|night)/i.test(text);
            if (!looksDarkOption || !looksThemeControl) continue;
  ...
  const toggleDarkModeForTab = async (tabId) => {
      ...
      try {
          if (isDarkMode) {
              ...
              const nativeResult = await tab.webview.executeJavaScript(nativeDarkModeScript);
              let fallbackNeeded = !!nativeResult?.fallbackNeeded;
              if (fallbackNeeded) {
                  await new Promise((resolve) => setTimeout(resolve, 250));
                  const retryResult = await tab.webview.executeJavaScript(nativeDarkModeScript);
                  fallbackNeeded = !!retryResult?.fallbackNeeded;
              }
              if (fallbackNeeded) {
                  await tab.webview.executeJavaScript(`
                      (function() {
  ...
  const launchOmChat = async () => {
      ...
      try {
          ...
          window.__omxOmChatNetworkUrl = appUrl;
          openAppTab(appUrl);
      } catch (error) {
  ...
  const bindSidebarNavigation = () => {
      ...
      bindClick(document.getElementById('btn-nav-home'), () => {
          handleSidebarHomeAction();
      });

      bindClick(document.getElementById('btn-nav-history'), () => {
          hideFeaturesHomePopup();
          openAppTab(HISTORY_URL);
      });

      bindClick(document.getElementById('btn-nav-settings'), () => {
          hideFeaturesHomePopup();
          openAppTab(SETTINGS_URL);
      });

      bindClick(document.getElementById('btn-nav-ai-chat'), async () => {
          hideFeaturesHomePopup();
          const targetUrl = await getAiChatTargetUrl();
          openAppTab(targetUrl);
      });

      bindClick(document.getElementById('btn-nav-downloads'), () => {
          hideFeaturesHomePopup();
          openAppTab(DOWNLOADS_URL);
      });

      bindClick(document.getElementById('btn-new-tab'), () => {
          searchSystem.handleNewTabRequest();
      });
  };

  const setBookmarkPanelVisible = (visible) => {
      if (!bookmarkTopPanel) return;
      bookmarkTopPanel.classList.toggle('hidden', !visible);
  };

  const setNavigationTopPanelVisible = (visible) => {
      if (!navigationTopPanel) return;
      navigationTopPanel.classList.toggle('hidden', !visible);
  };

  const setBookmarkEditorVisible = (visible) => {
      if (!bookmarkEditorOverlay) return;
      bookmarkEditorOverlay.classList.toggle('hidden', !visible);
  };

  const updateBookmarkEditorPreview = () => {
      const previewName = String(bookmarkEditorName?.value || '').trim() || 'New Bookmark';
      const previewIcon = sanitizeBookmarkIcon(bookmarkEditorIcon?.value || bookmarkFallbackIcon);
  ...
  const refreshBookmarkDraft = (tab = getActiveTabState()) => {
      return tab;
  };
  ...
  const renderBookmarkRows = () => {
      ...
      bookmarkItems.forEach((bookmark) => {
          ...
          bookmarkTopList.appendChild(row);
      });
  };
  ...
  const refreshBookmarks = async () => {
      ...
      try {
          ...
          bookmarkItems = Array.isArray(bookmarks) ? bookmarks.filter((entry) => entry?.url) : [];
          renderBookmarkRows();
      } catch (error) {
  ...
  const saveCurrentTabAsBookmark = async (tab = getActiveTabState()) => {
      ...
      try {
          bookmarkItems = await window.browserAPI.bookmarks.add({
              ...
              favicon: sanitizeBookmarkIcon(bookmarkEditorIcon?.value || tab.iconEl?.src || bookmarkFallbackIcon)
          });
          renderBookmarkRows();
  ...
  const openBookmarkEditor = (tab = getActiveTabState()) => {
      ...
      if (bookmarkEditorName) {
          bookmarkEditorName.value = String(tab.titleEl?.textContent || tab.url).trim() || tab.url;
      }
      if (bookmarkEditorIcon) {
          bookmarkEditorIcon.value = sanitizeBookmarkIcon(tab.iconEl?.src || bookmarkFallbackIcon);
      }
  ...
  const getLlamaServerUrl = () => {
      ...
      try {
          ...
          if (raw) {
              ...
              if (settings?.host) host = String(settings.host).trim() || host;
              if (settings?.port) port = String(settings.port).trim() || port;
          }
      } catch (_) {}
      ...
      if (cleanedHost.includes(':') && !cleanedHost.startsWith('[')) {
  ...
  const getYouTubeAddonSettings = (settings = cachedSettings) => {
      const youtubeAddon = settings?.youtubeAddon || {};
      return {
          enabled: youtubeAddon.enabled !== false,
          hideShorts: youtubeAddon.hideShorts !== false,
          hideHomeSuggestions: youtubeAddon.hideHomeSuggestions !== false,
          blurThumbnails: youtubeAddon.blurThumbnails === true,
          hideChats: youtubeAddon.hideChats === true,
          hideHeaderControls: youtubeAddon.hideHeaderControls !== false,
          blackAndWhiteMode: youtubeAddon.blackAndWhiteMode === true,
          cleanUi: youtubeAddon.cleanUi === true,
          hideAddonIcon: youtubeAddon.hideAddonIcon === true
      };
  };

  const getDuckAiChatSettings = (settings = cachedSettings) => {
      const aiChat = settings?.aiChat || {};
      return {
          hideSidebar: aiChat.duckAiHideSidebar === true
      };
  };
  ...
  const isYouTubeWatchUrl = (url = '') => {
      try {
          ...
          return String(parsed.pathname || '').startsWith('/watch');
      } catch (_) {
          return false;
      }
  };
  ...
  const canShowYouTubeAddon = () => {
      ...
      if (!yt.enabled) return false;
      return isYouTubeUrl(getActiveTabUrl());
  };

  const canShowDuckAiPanel = () => {
      return isDuckAiChatUrl(getActiveTabUrl());
  };
  ...
  const canShowPageInfoPanel = () => {
      ...
      return !(
          activeTab.isSystemPage
          || activeTab.isTextStudio
          || activeTab.isHistoryPage
          || activeTab.isGamesPage
          || activeTab.isDefensePage
          || activeTab.isHomePage
          || activeTab.isTodoPage
          || activeTab.isScraberPage
          || activeTab.isServerOperatorPage
      );
  };
  ...
  const setFeatureTileAvailability = (button, enabled, activeLabel, inactiveLabel) => {
      if (!button) return;
      button.title = enabled ? activeLabel : inactiveLabel;
      button.dataset.tooltip = enabled ? activeLabel : inactiveLabel;
      button.setAttribute('aria-disabled', enabled ? 'false' : 'true');
  };

  const syncSiteToolQuickLaunchButtons = () => {
      setFeatureTileAvailability(
          quickPanelYoutubeAddon,
          canShowYouTubeAddon(),
          'YouTube Addon',
          'Open YouTube to use'
      );
      setFeatureTileAvailability(
          ...
          canShowDuckAiPanel(),
          'Duck AI',
          'Open Duck AI chat to use'
      );
      setFeatureTileAvailability(
          quickPanelPageInfo,
          canShowPageInfoPanel(),
          'Page Info',
          'Unavailable on Om-X panels'
      );
  };
  ...
  const toggleYouTubeAddonPanel = () => {
      ...
      setBookmarkPanelVisible(false);
      setBookmarkEditorVisible(false);
      const isHidden = youtubeAddonPanel?.classList.contains('hidden');
      setYouTubeAddonVisible(isHidden);
  };
  ...
  const toggleDuckAiPanel = () => {
      ...
      setBookmarkPanelVisible(false);
      setBookmarkEditorVisible(false);
      const isHidden = duckAiPanel?.classList.contains('hidden');
      setDuckAiPanelVisible(isHidden);
  };
  ...
  const handleNavigationTopAction = (action) => {
      ...
      try {
          if (action === 'back' && activeWebview.canGoBack()) {
              activeWebview.goBack();
          } else if (action === 'forward' && activeWebview.canGoForward()) {
  ...
  const syncDuckAiPanel = () => {
      if (!duckAiPanel) return;
      const duckAi = getDuckAiChatSettings(cachedSettings);
      const canStayOpen = canShowDuckAiPanel();
      if (!canStayOpen) duckAiPanel.classList.add('hidden');
      if (duckAiToggleSidebar) duckAiToggleSidebar.checked = !!duckAi.hideSidebar;
  };
  ...
  const reloadActiveYouTubeWatchPage = () => {
      ...
      if (!isYouTubeWatchUrl(activeUrl)) return;
      try {
          activeWebview.reload();
      } catch (error) {
          console.warn('[YouTube Addon] Failed to reload active watch page:', error);
      }
  };
  ...
  const persistYouTubeAddonPreferences = async (partialYoutubeAddon = {}, options = {}) => {
      try {
          ...
          const previousYouTubeAddon = getYouTubeAddonSettings(cachedSettings);
          const nextYouTubeAddon = {
              ...(cachedSettings?.youtubeAddon || {}),
              ...partialYoutubeAddon
          };
          const shouldReloadAfterSave = options.reloadOnEnable === true
              && previousYouTubeAddon.cleanUi !== true
              && nextYouTubeAddon.cleanUi === true
              && isYouTubeWatchUrl(getActiveTabUrl());
          const nextSettings = {
              ...(cachedSettings || {}),
              youtubeAddon: nextYouTubeAddon
          };
          ...
          if (success && shouldReloadAfterSave) {
              window.setTimeout(reloadActiveYouTubeWatchPage, 0);
          }
      } catch (e) {
  ...
  const setupYouTubeAddonEvents = () => {
      ...
      youtubeAddonToggleHome?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ hideHomeSuggestions: youtubeAddonToggleHome.checked });
      });
      youtubeAddonToggleBlur?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ blurThumbnails: youtubeAddonToggleBlur.checked });
      });
      youtubeAddonToggleChat?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ hideChats: youtubeAddonToggleChat.checked });
      });
      youtubeAddonToggleUi?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ hideHeaderControls: youtubeAddonToggleUi.checked });
      });
      youtubeAddonToggleBw?.addEventListener('change', () => {
          persistYouTubeAddonPreferences({ blackAndWhiteMode: youtubeAddonToggleBw.checked });
      });
      youtubeAddonToggleCleanUi?.addEventListener('change', () => {
          persistYouTubeAddonPreferences(
  ...
  const setupDuckAiPanelEvents = () => {
      ...
      duckAiToggleSidebar?.addEventListener('change', () => {
          persistDuckAiPreferences({ duckAiHideSidebar: duckAiToggleSidebar.checked });
      });
  };
  ...
  const loadSettings = async (injectedSettings = null) => {
      try {
          ...
          applyFeatureVisibility({ enableHistory: true });
          syncYouTubeAddonPanel();
          syncDuckAiPanel();
          syncSiteToolQuickLaunchButtons();
  ...
  if (window.browserAPI?.system?.onLanIpResolved) {
      window.browserAPI.system.onLanIpResolved((ipAddress) => {
          ...
          window.dispatchEvent(new CustomEvent('omx:lan-ip-resolved', {
              detail: { ip: ipAddress }
          }));
      });
  }
  ...
  try {
      ...
      console.error('[Renderer] Failed to initialize SidePanel', error);
  }
  ...
  // Now initialize the UI components after TabManager is created
    try {
        ...
        console.error('[Renderer] Failed to initialize TranslatorUI', error);
    }

    try {
        writerUI = new WriterUI(tabManager);
    } catch (error) {
        console.error('[Renderer] Failed to initialize WriterUI', error);
    }

    try {
        screenshotOverlay = new ScreenshotOverlay(async (url) => window.browserAPI.downloads.start(url, { saveAs: true }));
    } catch (error) {
        console.error('[Renderer] Failed to initialize ScreenshotOverlay', error);
    }
    ...
    if (btnLens) {
        btnLens.addEventListener('click', (e) => {
            ...
            webview.capturePage().then((img) => {
                screenshotOverlay.start(img, openLensSearchForImage, { lensMode: true });
            }).catch((err) => {
  ...
  if (window.browserAPI.onShortcut) {
      window.browserAPI.onShortcut((command) => {
          if (command === 'new-tab') { 
              ...
              captureActiveWebview(openLensSearchForImage, { lensMode: true });
          }
      });
  }
  ...
  const btnNavAiChat = document.getElementById('btn-nav-ai-chat');
  ...
  if (quickPanelPageInfo) quickPanelPageInfo.onclick = () => {
      if (!canShowPageInfoPanel()) return;
      openSiteInfoPopup();
  };
  if (quickPanelYoutubeAddon) quickPanelYoutubeAddon.onclick = () => {
      if (!canShowYouTubeAddon()) return;
      hideFeaturesHomePopup();
      toggleYouTubeAddonPanel();
  };
  if (quickPanelDuckAi) quickPanelDuckAi.onclick = () => {
      if (!canShowDuckAiPanel()) return;
      hideFeaturesHomePopup();
      toggleDuckAiPanel();
  };
  ...
  bookmarkEditorOverlay?.addEventListener('mousedown', (event) => {
      if (event.target === bookmarkEditorOverlay) {
          setBookmarkEditorVisible(false);
          pendingBookmarkTab = null;
      }
  });
  ...
  bookmarkEditorName?.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' && pendingBookmarkTab) {
          event.preventDefault();
          saveCurrentTabAsBookmark(pendingBookmarkTab);
      }
  });
  bookmarkEditorIcon?.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' && pendingBookmarkTab) {
          event.preventDefault();
          saveCurrentTabAsBookmark(pendingBookmarkTab);
      }
  ...
  bookmarkEditorSave?.addEventListener('click', () => {
      if (pendingBookmarkTab) saveCurrentTabAsBookmark(pendingBookmarkTab);
  });
  ...
  document.addEventListener('mousedown', (e) => {
      ...
      if (featuresHomePopup && !featuresHomePopup.classList.contains('hidden')) {
          if (!featuresHomePopup.contains(e.target) && !btnNavHome?.contains?.(e.target)) {
              hideFeaturesHomePopup();
          }
      }
      if (navigationTopPanel && !navigationTopPanel.classList.contains('hidden')) {
          if (!navigationTopPanel.contains(e.target) && !quickPanelNavigation?.contains?.(e.target)) {
              setNavigationTopPanelVisible(false);
          }
      }
      if (bookmarkTopPanel && !bookmarkTopPanel.classList.contains('hidden')) {
          if (!bookmarkTopPanel.contains(e.target) && !quickPanelBookmarks?.contains?.(e.target)) {
              setBookmarkPanelVisible(false);
          }
      }
      if (youtubeAddonPanel && !youtubeAddonPanel.classList.contains('hidden')) {
          if (!youtubeAddonPanel.contains(e.target) && !btnNavHome?.contains?.(e.target) && !clickedWebview) {
              setYouTubeAddonVisible(false);
          }
      }
      if (duckAiPanel && !duckAiPanel.classList.contains('hidden')) {
          if (!duckAiPanel.contains(e.target) && !btnNavHome?.contains?.(e.target)) {
              setDuckAiPanelVisible(false);
          }
      }
      if (siteInfoOverlay && !siteInfoOverlay.classList.contains('hidden')) {
          if (!siteInfoOverlay.contains(e.target)) {
              hideSiteInfoPopup();
          }
      }
  });
  ...
  const createFittedImagePdf = (jsPDFCtor, canvas) => {
      ...
      pdf.addImage(jpegDataUrl, 'JPEG', x, y, drawWidth, drawHeight);
      return pdf;
  };
      ...
      btn.onclick = async () => {
          ...
          try {
              if (format === 'pdf') {
                  ...
                  await window.browserAPI.downloads.start(dataUrl, { saveAs: true, filename });
              }
          } catch (e) {
              console.error('[Image Download] Export failed:', e);
              if (format !== 'pdf') {
                  await window.browserAPI.downloads.start(src, { saveAs: true });
              }
          }
      };
  });
...
(async () => {
  ...
  const captureActiveWebview = (onCaptureOverride = null, overlayOptions = {}) => {
      ...
      if (screenshotDelaySeconds > 0) {
          screenshotOverlay.startDelayCapture(
              screenshotDelaySeconds,
              captureFn,
              onCaptureOverride,
              { ...overlayOptions, captureBounds }
          );
      } else {
          captureFn()
