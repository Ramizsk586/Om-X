(() => {
  ...
  const ACCOUNT_AI_STEP_DELAY_MS = 220;
  const ACCOUNT_AI_CALL_DELAY_MS = 260;
  ...
  const TREE_HEAVY_DIRS = new Set(['node_modules', '.git', '.cache', '.next', 'dist', 'build', 'coverage', 'out', 'electron']);
  const TREE_MAX_ITEMS_PER_DIR = 1200;
  ...
  const ALLOWED_AI_FUNCTIONS = new Set([]);
  const AI_CHAT_ENABLED = false;
  const SIDE_PANEL_MODES = ['explorer', 'search', 'source'];
  ...
  const warnedTreeLimitDirs = new Set();
  ...
  function injectRuntimeStyles() {
    ...
    style.textContent = `
      ...
      .workspace.side-mode-account .explorer {
        display: none !important;
      }
      .activity-panel {
        grid-column: 2;
      ...
      .workspace.side-mode-account .panel-account {
        display: grid;
      }
      ...
      .account-ai-select:focus,
      .account-ai-model:focus,
      .account-ai-input:focus {
        border-color: #0a84ff;
      ...
      .account-ai-line.user {
        color: #d8ecff;
      }
      .account-ai-line.tool {
        color: #cfd3ff;
      }
      .account-ai-line-head {
        flex: 0 0 auto;
        display: inline-flex;
        align-items: center;
        gap: 5px;
      ...
      .account-ai-line-role {
        font-weight: 700;
        color: #aeb8c8;
      }
      .account-ai-line-time {
        color: #7f8a9b;
        letter-spacing: 0;
        text-transform: none;
      }
      .account-ai-line-body {
        margin: 0;
  ...
  function bindEvents() {
    ...
    onClick('btn-rail-run-panel', () => void runActiveFile());
  ...
  function renderSidePanelMode() {
    ...
    if (mode === 'extensions' || mode === 'extension' || mode === 'account') {
    ...
    return `
      <div class="ext-contrib-panel">
        <div class="side-panel-card">
          ...
          <p>Coder detected this activity-bar contribution from <strong>${escapeHtml(ext?.displayName || item.extensionId)}</strong>.</p>
  ...
  function getRunPanelLaunchPath() {
    ...
    if (!root) return '';
    return joinPath(root, '.omx', 'launch.json');
  }
  ...
  function getRunPanelProfile() {
    ...
    if (ext === 'html' || ext === 'htm') {
      ...
      profile.description = 'Built-in sandboxed HTML preview is available. Debug sections mirror the Coder layout while debugger APIs are still in progress.';
    ...
    if (['ts', 'tsx', 'jsx'].includes(ext)) {
      ...
      profile.supportLabel = 'Setup Required';
      profile.supportTone = 'warn';
      profile.description = 'Configure a build/run task and launch.json. The panel matches the Coder workflow, but execution backends are still limited in Coder.';
      profile.debugTerminalRecommended = true;
      profile.browserUrlHelpful = true;
  ...
  function renderRunPanelHtml() {
    ...
    const launchPathLabel = launchPath ? (toProjectRelative(launchPath) || launchPath) : '.omx/launch.json';
    const hasWorkspace = Boolean(state.projectRoot);
    ...
    const watchBody = `
      <p class="run-empty-note">No watch expressions yet. This section is included to match the built-in debug layout.</p>
    ...
    const breakpointsBody = `
      ...
      <div class="run-kv">
        <div class="run-kv-row">
          ...
          <div class="run-kv-value">Use <code>${escapeHtml(launchPathLabel)}</code> to prepare workspace debug configs now.</div>
        </div>
      </div>
    `;
    ...
    return `
      <div class="side-panel-stack run-panel">
        ...
        <div class="side-panel-card">
          ...
          <p>${hasWorkspace
            ? `Create or open <code>${escapeHtml(launchPathLabel)}</code> for workspace debug configurations.`
            : 'Open a project folder first to create .omx/launch.json in the workspace root.'}</p>
          <div class="side-tools" style="margin-top:8px;">
  ...
  async function createOrOpenRunLaunchJson() {
    ...
    const omxLaunchDir = joinPath(root, '.omx');
    const launchPath = joinPath(omxLaunchDir, 'launch.json');

    try {
      const dirStat = await api.files?.stat?.(omxLaunchDir);
      if (!dirStat?.exists) {
        const madeDir = await api.files?.createFolder?.(omxLaunchDir);
        if (!madeDir) {
          showToast('Failed to create .omx folder', 'error');
          markRunPanelLastAction('Failed to create .omx folder', 'error');
          if (state.sidePanelMode === 'run') renderSidePanelMode();
          return false;
        }
      }
      ...
      if (!launchStat?.exists) {
        ...
        if (!ok) {
          ...
          markRunPanelLastAction('Failed to create .omx/launch.json', 'error');
          if (state.sidePanelMode === 'run') renderSidePanelMode();
          return false;
        }
        created = true;
  ...
  function ensureAccountAiState() {
    if (!state.accountAi || typeof state.accountAi !== 'object') {
      state.accountAi = {
        loading: false,
        loaded: false,
        busy: false,
    ...
    if (!Array.isArray(state.accountAi.providers)) state.accountAi.providers = [];
    if (!Array.isArray(state.accountAi.messages)) state.accountAi.messages = [];
    return state.accountAi;
  }

  function focusAccountAiInput(selectEnd = true) {
    const input = document.getElementById('account-ai-input');
    if (!input) return;
    try {
      input.focus({ preventScroll: true });
    } catch (_) {
  ...
  async function refreshAccountAiProviders(options = {}) {
    const account = ensureAccountAiState();
    if (account.loading) return;
    if (account.loaded && options.force !== true) return;
    account.loading = true;
    try {
      const settings = await api.settings?.get?.();
      const providersObj = (settings && typeof settings.providers === 'object') ? settings.providers : {};
      const providers = Object.entries(providersObj).map(([id, cfg]) => {
        ...
        return {
          ...
          label: model ? `${providerId} (${model})` : providerId
        };
      }).filter((x) => x.id);
      providers.sort((a, b) => a.id.localeCompare(b.id));
      account.providers = providers;
      const activeProvider = String(settings?.activeProvider || '').trim();
      const preferred = String(account.selectedProvider || '').trim();
      const nextProvider = providers.some((p) => p.id === preferred)
        ? preferred
        : (providers.some((p) => p.id === activeProvider) ? activeProvider : (providers[0]?.id || ''));
      account.selectedProvider = nextProvider;
      const selectedCfg = providers.find((p) => p.id === nextProvider);
      account.selectedModel = String(account.selectedModel || selectedCfg?.model || '').trim();
      account.loaded = true;
    } catch (error) {
      appendAccountAiMessage('assistant', `Failed to load saved AI providers: ${error?.message || error}`);
    } finally {
      account.loading = false;
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();
    }
  }

  function getSelectedAccountProviderConfig() {
    const account = ensureAccountAiState();
    const providerId = String(account.selectedProvider || '').trim();
    if (!providerId) return null;
    const found = account.providers.find((p) => p.id === providerId);
    if (!found) return null;
    return {
      provider: providerId,
      model: String(account.selectedModel || found.model || '').trim(),
      key: String(found.key || '').trim(),
      baseUrl: String(found.baseUrl || '').trim()
    };
  }

  function renderAccountAiMessagesHtml(account) {
    const messages = Array.isArray(account?.messages) ? account.messages : [];
    if (!messages.length) {
      return `
        <div class="account-ai-empty">
          Ask for code exploration, file snippets, search tasks, or shell commands.
          The assistant will use your saved provider key and model configuration.
        </div>
      `;
    }
    return `
      <div class="account-ai-feed">
        ${messages.map((msg) => {
          const roleStr = String(msg.role || 'assistant').toLowerCase();
          const textStr = String(msg.text || '');
          return `
          <div class="account-ai-line ${escapeHtml(roleStr)}">
            <div class="account-ai-line-head">
              <span class="account-ai-line-role">${escapeHtml(String(msg.role || 'assistant'))}</span>
              <span class="account-ai-line-time">${escapeHtml(formatClock(msg.ts || Date.now()))}</span>
            </div>
            <pre class="account-ai-line-body">${escapeHtml(textStr)}</pre>
          </div>
        `;
        }).join('')}
      </div>
  ...
  function renderAccountAiPanelHtml() {
    return `
      <div class="account-ai-shell">
        <div class="account-ai-empty">
          AI assistant features were removed from Coder.
        </div>
      </div>
    `;
  }

  function appendAccountAiMessage(role, text) {
    const account = ensureAccountAiState();
    ...
    if (account.messages.length > 80) {
      account.messages = account.messages.slice(account.messages.length - 80);
    }
    if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();
    scheduleSessionSave();
  }

  async function handleAccountAiSideAction(action) {
    if (String(action || '').startsWith('account-ai-')) {
      showToast('AI assistant has been removed from Coder', 'warn');
      return true;
    }
    const account = ensureAccountAiState();
    if (action === 'account-ai-refresh') {
      await refreshAccountAiProviders({ force: true });
      return true;
    }
    if (action === 'account-ai-clear') {
      account.messages = [];
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();
      scheduleSessionSave();
      return true;
    }
    if (action === 'account-ai-send') {
      await sendAccountAiPrompt();
      return true;
    }
    if (action === 'account-ai-apply-staged') {
      await approvePendingAiChanges();
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();
      return true;
    }
    if (action === 'account-ai-discard-staged') {
      rejectPendingAiChanges();
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();
      return true;
    }
    return false;
  ...
  function parseAccountAiResponse(response) {
    const raw = String(response?.text || '').trim();
    const nativeCalls = Array.isArray(response?.functionCalls)
      ? response.functionCalls.map((fc) => normalizeAccountAiCall({ function: fc?.name, arguments: fc?.args || {} }))
      : [];
    if (nativeCalls.length) {
      return {
        message: sanitizeAiVisiblePlanMessage(raw) || '',
        calls: nativeCalls.filter((call) => call.function)
      };
    }
    const parsed = parseJsonLoose(raw);
    if (!parsed) return { message: raw, calls: [] };
    let callsRaw = Array.isArray(parsed?.calls)
      ? parsed.calls
      : (Array.isArray(parsed?.functionCalls) ? parsed.functionCalls : []);
    if (!Array.isArray(callsRaw) && Array.isArray(parsed?.actions)) callsRaw = parsed.actions;
    if (!Array.isArray(callsRaw) && Array.isArray(parsed?.toolCalls)) callsRaw = parsed.toolCalls;
    if (!Array.isArray(callsRaw) && Array.isArray(parsed?.edits)) {
      callsRaw = parsed.edits.map((edit) => {
        if (!edit || typeof edit !== 'object') return null;
        const kind = String(edit.type || edit.action || '').toLowerCase();
        if (kind === 'delete') {
          return { function: 'deleteFile', arguments: { filePath: edit.filePath || edit.path } };
        }
        if (kind === 'rename') {
          return { function: 'renameFile', arguments: { oldPath: edit.oldPath || edit.from, newPath: edit.newPath || edit.to } };
        }
        if (kind === 'create') {
          return { function: 'createFile', arguments: { filePath: edit.filePath || edit.path, content: edit.content || edit.newContent || '' } };
        }
        return {
          function: 'applyEdit',
          arguments: {
            filePath: edit.filePath || edit.path,
            rangeOrSelector: edit.rangeOrSelector || edit.range || edit.selector || { type: 'full' },
            newContent: edit.newContent ?? edit.content ?? edit.replacement ?? ''
          }
        };
      }).filter(Boolean);
    }
    const calls = (Array.isArray(callsRaw) ? callsRaw : [])
      .map((call) => normalizeAccountAiCall(call))
      .filter((call) => call.function);
    const message = sanitizeAiVisiblePlanMessage(parsed?.message || parsed?.summary || '') || (calls.length ? '' : raw);
    return { message, calls };
  }

  function normalizeAccountAiFunctionName(name) {
    ...
    const aliases = {
      applyedit: 'applyEdit',
      editfile: 'applyEdit',
      modifyfile: 'applyEdit',
      updatefile: 'applyEdit',
      replaceinfile: 'applyEdit',
      patchfile: 'applyEdit',
      writefile: 'applyEdit',
    ...
    return aliases[lowered] || fn;
  }

  function normalizeAccountAiCall(rawCall) {
    if (!rawCall || typeof rawCall !== 'object') return { function: '', arguments: {} };
    const normalizedFn = normalizeAccountAiFunctionName(rawCall.function || rawCall.name || rawCall.tool || rawCall.operation || '');
    const args = (rawCall.arguments && typeof rawCall.arguments === 'object')
      ? { ...rawCall.arguments }
      : ((rawCall.args && typeof rawCall.args === 'object') ? { ...rawCall.args } : {});
    if (normalizedFn === 'applyEdit') {
      if (!args.filePath) {
        args.filePath = args.path || args.file || args.targetPath || args.target || args.filename || args.fileName || '';
      }
      if (args.newContent == null) {
        const altContentKeys = [
          'content', 'replacement', 'text', 'updatedContent', 'after',
          'afterContent', 'newText', 'newCode', 'code', 'value'
        ];
        for (const key of altContentKeys) {
          if (args[key] != null) {
            args.newContent = args[key];
            break;
          }
        }
      }
      if (args.newContent == null && args.edit && typeof args.edit === 'object') {
        const edit = args.edit;
        if (edit.newContent != null) args.newContent = edit.newContent;
        else if (edit.content != null) args.newContent = edit.content;
        else if (edit.replacement != null) args.newContent = edit.replacement;
        else if (edit.newText != null) args.newContent = edit.newText;
        if (!args.filePath) args.filePath = edit.filePath || edit.path || args.filePath;
        if (!args.rangeOrSelector) args.rangeOrSelector = edit.rangeOrSelector || edit.range || edit.selector || null;
      }
      if (!args.rangeOrSelector) {
        if (args.range && typeof args.range === 'object') args.rangeOrSelector = args.range;
        else if (args.selector && typeof args.selector === 'object') args.rangeOrSelector = args.selector;
        else if (typeof args.oldText === 'string' && args.oldText.length > 0) {
          args.rangeOrSelector = { type: 'search', pattern: args.oldText, occurrence: Number(args.occurrence) || 1 };
        } else if (typeof args.find === 'string' && args.find.length > 0) {
          args.rangeOrSelector = { type: 'search', pattern: args.find, occurrence: Number(args.occurrence) || 1 };
        } else if (typeof args.pattern === 'string' && args.pattern.length > 0) {
          args.rangeOrSelector = { type: 'search', pattern: args.pattern, occurrence: Number(args.occurrence) || 1 };
        } else if (Number.isFinite(Number(args.line))) {
          const line = Math.max(1, Number(args.line));
          args.rangeOrSelector = { type: 'line', startLine: line, endLine: line };
        } else if (Number.isFinite(Number(args.fromLine)) || Number.isFinite(Number(args.toLine))) {
          const start = Math.max(1, Number(args.fromLine) || Number(args.toLine) || 1);
          const end = Math.max(start, Number(args.toLine) || start);
          args.rangeOrSelector = { type: 'line', startLine: start, endLine: end };
        } else if (Number.isFinite(Number(args.start)) && Number.isFinite(Number(args.end))) {
          args.rangeOrSelector = { type: 'offset', start: Number(args.start), end: Number(args.end) };
        } else if (Number.isFinite(Number(args.startLine)) && Number.isFinite(Number(args.endLine))) {
          args.rangeOrSelector = { type: 'line', startLine: Number(args.startLine), endLine: Number(args.endLine) };
        } else {
          args.rangeOrSelector = { type: 'full' };
        }
      }
    } else if (normalizedFn === 'createFile' || normalizedFn === 'deleteFile') {
      if (!args.filePath && args.path) args.filePath = args.path;
    } else if (normalizedFn === 'renameFile') {
      if (!args.oldPath && args.from) args.oldPath = args.from;
      if (!args.newPath && args.to) args.newPath = args.to;
    } else if (normalizedFn === 'getFile') {
      if (!args.filePath && args.path) args.filePath = args.path;
    }
    return { function: normalizedFn, arguments: args };
  }
  ...
  function requestLikelyNeedsEdits(promptText) {
    const text = String(promptText || '').toLowerCase();
    return /(edit|update|modify|change|fix|refactor|improve|create|build|implement|add|remove|replace|rewrite)/.test(text);
  }

  function describeAccountAiCallForFeed(call) {
    const fn = String(call?.function || '').trim();
    const args = call?.arguments && typeof call.arguments === 'object' ? call.arguments : {};
    if (!fn) return '';
    if (fn === 'applyEdit') {
      const path = toProjectRelative(String(args.filePath || '')) || String(args.filePath || 'file');
      return `Edit ${path}`;
    }
    if (fn === 'createFile') {
      const path = toProjectRelative(String(args.path || args.filePath || '')) || String(args.path || args.filePath || 'file');
      return `Create ${path}`;
    }
    if (fn === 'deleteFile') {
      const path = toProjectRelative(String(args.path || args.filePath || '')) || String(args.path || args.filePath || 'file');
    ...
    if (fn === 'renameFile') {
      const oldPath = toProjectRelative(String(args.oldPath || '')) || String(args.oldPath || 'file');
      const newPath = toProjectRelative(String(args.newPath || '')) || String(args.newPath || 'file');
      return `Rename ${oldPath} -> ${newPath}`;
    }
    return '';
  }

  function formatAccountAiEditsFeed(lines, title) {
    const safeTitle = String(title || 'Edits');
    const body = Array.isArray(lines) ? lines.filter(Boolean) : [];
    if (!body.length) return '';
    return `${safeTitle}:\n${body.map((line) => `- ${line}`).join('\n')}`;
  }

  async function executeAccountAiCall(call, simulator) {
    const normalized = normalizeAccountAiCall(call);
    const fn = String(normalized.function || '').trim();
    const args = normalized.arguments || {};
    if (!fn) return 'Skipped empty function call.';
    if (fn === 'runCommand') {
      const command = String(args.command || args.cmd || '').trim();
      if (!command) throw new Error('runCommand requires command');
      const cwd = resolveWorkspacePath(args.cwd || state.projectRoot || '');
      if (cwd && !isPathAllowed(cwd)) throw new Error('runCommand cwd is outside project');
      const timeoutMs = clamp(Number(args.timeoutMs) || 20000, 2000, 120000);
      ...
      const stdout = truncateText(String(result?.stdout || '').trim(), 1800);
      const stderr = truncateText(String(result?.stderr || '').trim(), 1200);
      const prefix = result?.success === false ? 'Command failed' : 'Command completed';
      return `${prefix}: ${command}\n${stdout ? `stdout:\n${stdout}` : ''}${stderr ? `\nstderr:\n${stderr}` : ''}`.trim();
    }
    if (fn === 'listFiles') {
      const args = call.arguments || {};
      const targetDir = resolveWorkspacePath(args.directory || args.path || state.projectRoot || '');
      if (!targetDir) throw new Error('listFiles requires an open folder or directory path');
      if (!isPathAllowed(targetDir)) throw new Error('listFiles directory is outside project scope');
      const maxFiles = clamp(Number(args.maxFiles) || 200, 1, 800);
      const files = await listFilesInDirectory(targetDir, { recursive: true, maxFiles });
      const relativeFiles = files.map((p) => toProjectRelative(p) || p);
      const shown = relativeFiles.slice(0, 120);
      const extra = relativeFiles.length - shown.length;
      return `${relativeFiles.length} file(s):\n${shown.join('\n')}${extra > 0 ? `\n... +${extra} more` : ''}`;
    }
    if (fn === 'getFile') {
      const args = call.arguments || {};
      const filePath = resolveWorkspacePath(args.filePath || args.path || '');
      if (!filePath) throw new Error('getFile requires filePath');
      if (!isPathAllowed(filePath)) throw new Error('getFile path is outside project scope');
      ...
      const relPath = toProjectRelative(filePath) || filePath;
      let startLine = 1;
      let endLine = countLines(current);
      const range = args.rangeOrSelector;
      if (range && String(range.type || '').toLowerCase() === 'line') {
        startLine = Math.max(1, Number(range.startLine) || 1);
        endLine = Math.max(startLine, Number(range.endLine) || startLine);
      }
      return `Read ${relPath} ${Math.max(0, startLine - 1)} - ${Math.max(0, endLine - 1)}`;
    }
    ...
    const staged = await stageAiCalls([{ function: fn, arguments: args }], simulator);
    if (staged.readLogs.length) {
      staged.readLogs.forEach((line) => addAiLog(line, 'info'));
    }
    if (staged.actions.length) {
      if (state.pendingAiBatch?.actions && Array.isArray(state.pendingAiBatch.actions)) {
        state.pendingAiBatch.actions.push(...staged.actions);
      } else {
        state.pendingAiBatch = {
          id: `batch-${Date.now()}`,
          prompt: 'Account AI staged actions',
          createdAt: Date.now(),
          assistantMessage: '',
          actions: staged.actions.slice()
        };
      }
      renderPendingAi();
      const filesTouched = [...new Set(staged.actions.map((a) => a.filePath || a.newPath || a.oldPath).filter(Boolean))];
      return `Staged ${staged.actions.length} edit call(s) across ${filesTouched.length} file(s). Use "Apply Staged Edits" to commit.`;
    }
    if (staged.errors.length) {
      throw new Error(staged.errors.join('\n'));
    }
    return staged.readLogs.length ? staged.readLogs.join('\n') : `Executed ${fn}`;
  }

  /**
   * Helper: Wrap API call with timeout protection
   * @param {Promise} apiPromise - The API call promise
   * @param {number} timeoutMs - Timeout in milliseconds (default 15000)
   * @param {string} stepName - Name of the step for error messaging
   * @returns {Promise} - Response or timeout error
   */
  async function executeAiCallWithTimeout(apiPromise, timeoutMs = 15000, stepName = 'AI Response') {
    ...
    timerMsg.id = timerId;
    timerMsg.style.cssText = 'font-size:11px;color:#a8b5d1;margin:4px 0;font-family:monospace;';
    timerMsg.textContent = `⏱️  Waiting (${Math.round(timeoutMs / 1000)}s timeout)...`;
    const chatEl = els['ai-chat'];
    if (chatEl) chatEl.appendChild(timerMsg);
    
    // Update timer every 500ms
    timerInterval = setInterval(() => {
      remainingMs -= 500;
      const secondsLeft = Math.max(0, Math.round(remainingMs / 1000));
      if (timerMsg && secondsLeft > 0) {
        timerMsg.textContent = `⏱️  Waiting (${secondsLeft}s)...`;
      }
    }, 500);
    ...
    try {
      ...
      if (timerMsg) {
        timerMsg.textContent = `✓ Response received in ${Math.round((timeoutMs - remainingMs) / 1000)}s`;
        timerMsg.style.color = '#8df0c0';
      }
      
      return result;
    } finally {
      if (timerInterval) clearInterval(timerInterval);
      // Clean up timer message after a short delay
      if (timerMsg) {
        setTimeout(() => timerMsg.remove?.(), 2000);
      }
    }
  }

  /**
   * ERROR LINE INDICATOR SYSTEM
   * Tracks code errors, shows indicators on error lines, and provides AI explanations
   */

  // Store detected errors by file
  const codeErrorMap = new Map(); // filePath -> [{ lineNumber, column, message, severity, code }]
  const errorIndicatorElements = new Map(); // unique key -> DOM element

  /**
   * Parse error patterns from terminal output or compiler messages
   * Detects: "filename:line:col: error: message"
   */
  function parseErrorFromOutput(output) {
    const errors = [];
    const lines = String(output || '').split('\n');
    
    // Common error patterns: file:line:col: error/warning: message
    const patterns = [
      /^(.+?):(\d+):(\d+):\s*(error|warning):\s*(.+)$/,     // GCC/Clang style
      /^(.+?):(\d+):(\d+)\s*-\s*(.+?)\s*-\s*(.+)$/,           // Some compilers
      /^Error in (.+?) at line (\d+):\s*(.+)$/,               // Generic
      /^(.+?)\((\d+),(\d+)\):\s*(error|warning)\s*(.+)$/      // MSVC style
    ];
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          errors.push({
            ...
            message: match[5] || match[4] || 'Unknown error',
  ...
  /**
   * Register errors detected in a file
   */
  function registerFileErrors(filePath, errors) {
    if (!errors || errors.length === 0) {
      codeErrorMap.delete(filePath);
      return;
    }
    codeErrorMap.set(filePath, errors);
  }
  ...
  function getFileErrors(filePath) {
    return codeErrorMap.get(filePath) || [];
  }

  /**
   * Create error line indicator UI
   */
  function createErrorLineIndicator(error, filePath) {
    const indicatorId = `err-line-${filePath}-${error.lineNumber}`;
    
    const indicator = document.createElement('span');
    indicator.id = indicatorId;
    indicator.className = 'error-line-indicator';
    indicator.title = error.message;
    indicator.innerHTML = error.severity === 'error' ? '⚠️' : '💡';
    ...
    indicator.addEventListener('click', async (e) => {
      e.stopPropagation();
      await requestErrorExplanation(filePath, error);
    });
  ...
  /**
   * Request AI explanation and solution for an error
   */
  async function requestErrorExplanation(filePath, error) {
    try {
      const account = ensureAccountAiState();
      const cfg = getSelectedAccountProviderConfig();
      
      if (!cfg?.provider) {
        showToast('No AI provider configured', 'warn');
        return;
      }
      ...
      appendAccountAiMessage('tool', `🔍 Analyzing error at ${filePath}:${error.lineNumber}...`);
      
      // Read the file around the error line
      let codeContext = '';
      try {
        if (api.files?.read) {
          await api.files?.allowPath?.(filePath, 'file');
          const fullContent = await api.files?.read?.(filePath);
          const allLines = String(fullContent || '').split('\n');
          
          // Get context lines (±5 lines around error)
          const startLine = Math.max(0, error.lineNumber - 6);
          const endLine = Math.min(allLines.length, error.lineNumber + 4);
        ...
        codeContext = `[Could not read file: ${readErr.message}]`;
      }

      // Request AI analysis
      const errorExplainPrompt = JSON.stringify({
        ...
        instruction: 'Analyze this error and provide: 1) Clear explanation 2) Root cause 3) Specific solutions with code examples. Format with clear sections.'
      }, null, 2);

      const response = await executeAiCallWithTimeout(
        api.ai?.performTask?.({
          promptOverride: errorExplainPrompt,
          systemInstruction: [
            'You are a code debugging expert in Coder IDE.',
            'Analyze the provided code error and explain:',
            '1. EXPLANATION: What the error message means',
            '2. ROOT CAUSE: Why this error occurred in this code',
            '3. SOLUTIONS: Specific fixes with code examples',
            'Format clearly with markdown headers. Be concise and actionable.'
          ].join('\n'),
          context: 'writer',
          searchMode: false,
          configOverride: cfg
      ...
      if (response?.text) {
        const explanation = String(response.text || '');
        
        // Parse explanation into sections
        const sections = parseErrorExplanation(explanation);
        
        // Format and display the explanation
        if (window.aiOutputFormatter?.formatErrorExplanation) {
          const formatted = window.aiOutputFormatter.formatErrorExplanation({
            errorMessage: error.message,
            filePath: filePath,
            lineNumber: error.lineNumber,
            explanation: sections.rootCause || explanation,
            solutions: sections.solutions || []
          });
          appendAccountAiMessage('assistant', formatted);
        } else {
          appendAccountAiMessage('assistant', `**Error Analysis for ${filePath}:${error.lineNumber}**\n\n${explanation}`);
        }

        // Offer to create a fix
        appendAccountAiMessage('tool', '💡 Would you like me to suggest a fix? Say "fix" or describe what you want to change.');
      } else {
        appendAccountAiMessage('assistant', '⚠️ Unable to analyze error. Please check the error message and try again.');
      }

    } catch (err) {
      appendAccountAiMessage('assistant', `Error analyzing code: ${err?.message || err}`);
    } finally {
      account.busy = false;
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) {
        renderSidePanelMode();
      }
    }
  }
  ...
  /**
   * Parse error explanation into structured sections
   */
  function parseErrorExplanation(explanation) {
    const sections = {
      explanation: '',
      rootCause: '',
      solutions: []
    ...
    const explanationMatch = text.match(/(?:EXPLANATION|What.*?:)\s*\n?([\s\S]*?)(?=\n(?:ROOT CAUSE|Why|SOLUTION|$))/i);
    if (explanationMatch) sections.explanation = explanationMatch[1].trim();
    
    const causeMacth = text.match(/(?:ROOT CAUSE|Why.*?:)\s*\n?([\s\S]*?)(?=\n(?:SOLUTION|Fix|$))/i);
    if (causeMacth) sections.rootCause = causeMacth[1].trim();
    
    // Extract solutions (numbered list or bullet points)
    const solutionsMatch = text.match(/(?:SOLUTION|FIX|FIXES|Suggestions?)([\s\S]*?)$/i);
    if (solutionsMatch) {
      const solText = solutionsMatch[1];
      const solutions = solText
        .split(/\n(?:\d+\.|[-•*])\s*/i)
        .map(s => s.trim())
        .filter(s => s && s.length > 5);
      
      sections.solutions = solutions.slice(0, 5); // Limit to 5 solutions
    }

    // Fallback: if no structured sections, use full text
    if (!sections.rootCause && !sections.explanation) {
      sections.explanation = text.split('\n').slice(0, 2).join(' ');
  ...
  /**
   * Process terminal output for errors and register them
   * Hook this to terminal event listeners
   */
  function processTerminalOutputForErrors(output) {
    const errors = parseErrorFromOutput(output);
    
    if (errors.length === 0) return;

    errors.forEach(error => {
      const existing = getFileErrors(error.filePath);
      const newErrors = [...existing, error];
      registerFileErrors(error.filePath, newErrors);
    });

    // Notify user about detected errors
    if (errors.length > 0) {
      const errorSummary = errors
        .slice(0, 3)
        .map(e => `${e.filePath}:${e.lineNumber} - ${e.message}`)
        .join(' | ');
      
      // Optional: show in AI panel
      if (els['ai-chat']) {
        appendAccountAiMessage('tool', `🚨 Detected ${errors.length} error(s): ${errorSummary}${errors.length > 3 ? '...' : ''}`);
      }
    }
  }

  /**
   * Enhanced AI Prompt Handler
   * Integrates all API contexts, professional formatting, and advanced execution
   */
  async function sendAccountAiPromptEnhanced() {
    showToast('AI assistant has been removed from Coder', 'warn');
    return;
    const account = ensureAccountAiState();
    if (account.busy) return;
    
    const prompt = String(account.draft || '').trim();
    if (!prompt) {
    ...
    const cfg = getSelectedAccountProviderConfig();
    if (!cfg?.provider) {
      showToast('No saved AI provider selected', 'warn');
      return;
    }

    account.busy = true;
    appendAccountAiMessage('user', prompt);
    account.draft = '';
    if (state.sidePanelMode === 'account' && state.explorerOpen !== false) renderSidePanelMode();

    try {
      if (!api.ai?.performTask) {
        throw new Error('AI task API unavailable');
      }

      // 1. Analyze capabilities
      const apiReport = await window.aiApiContext?.getApiReport?.();
      const advisor = window.apiAdvisor;
      const analysis = advisor?.analyzeTask?.(prompt);

      // 2. Show analysis
      appendAccountAiMessage('tool', '📊 Analyzing task requirements...');
      if (analysis?.recommendations?.length > 0) {
        const recText = analysis.recommendations
          .slice(0, 3)
          .map((r, i) => `${i + 1}. ${r.api} (${(r.confidence * 100).toFixed(0)}%)`)
          .join(' • ');
        appendAccountAiMessage('tool', `📈 API recommendations: ${recText}`);
      }

      // 3. Build enhanced context
      appendAccountAiMessage('tool', '🔍 Collecting workspace context...');
      ...
      appendAccountAiMessage('tool', `✓ Context ready: ${indexedFiles} files indexed, APIs: ${apiReport?.availableApis?.join(', ') || 'standard'}`);
      await waitMs(ACCOUNT_AI_STEP_DELAY_MS);

      // 4. Plan execution
      appendAccountAiMessage('tool', '📋 Planning execution strategy...');
      const executionPlan = [];
      
      if (analysis?.recommendations?.some(r => r.api === 'files')) {
        executionPlan.push({ api: 'files', weight: 0.9 });
      }
      if (analysis?.recommendations?.some(r => r.api === 'terminal')) {
        executionPlan.push({ api: 'terminal', weight: 0.8 });
      }
      if (analysis?.recommendations?.some(r => r.api === 'extensions')) {
        executionPlan.push({ api: 'extensions', weight: 0.7 });
      }

      appendAccountAiMessage('tool', `✓ Strategy ready with ${executionPlan.length} API endpoint(s)`);
      await waitMs(ACCOUNT_AI_STEP_DELAY_MS);

      // 5. Request AI processing
      appendAccountAiMessage('tool', '🤖 Requesting AI processing...');
      
      const systemInstruction = [
        'You are a professional CLI-style coding assistant in Coder IDE.',
        'Work intelligently using available APIs: files, terminal, extensions, git.',
        'Provide professional, concise responses with code blocks and suggestions.',
        'Return strict JSON for tool calls. Plain text for explanations.',
        `Available APIs: ${apiReport?.availableApis?.join(', ') || 'files, terminal, extensions'}`,
        'Always explain your approach before and after executing tasks.',
        'Format code blocks clearly with language hints.',
        'Include success metrics, timing, and next steps in responses.'
      ].join('\n');

      const requestPrompt = JSON.stringify({
        userRequest: prompt,
        workspace: workspaceSummary,
        availableApis: apiReport?.availableApis,
        executionPlan,
        mode: 'professional-cli',
        requiresProfessionalFormatting: true
      }, null, 2);

      const responsePromise = api.ai?.performTask?.({
        promptOverride: requestPrompt,
        systemInstruction,
        context: 'writer',
        searchMode: false,
        configOverride: cfg
      ...
      const response = await executeAiCallWithTimeout(responsePromise, 18000, 'Main AI Processing');

      if (response?.error) throw new Error(String(response.error));

      const parsed = parseAccountAiResponse(response || {});
      
      // 6. Stream response with animation
      if (parsed.message) {
        appendAccountAiMessage('assistant', parsed.message);
        if (window.aiCliEnhanced?.streamText) {
          const container = document.querySelector('.ai-chat-enhanced');
          const lastMsg = container?.lastElementChild;
          if (lastMsg) {
            window.aiCliEnhanced.streamText(lastMsg, parsed.message, { delay: 10 });
          }
        }
      }

      let calls = Array.isArray(parsed.calls) ? parsed.calls : [];
      
      // If no calls returned, request concrete edits
      if (!calls.length && requestLikelyNeedsEdits(prompt)) {
        appendAccountAiMessage('tool', '📝 Requesting concrete edits...');
        const mutatePrompt = JSON.stringify({
          userRequest: prompt,
          observations: String(parsed.message || response?.text || '').trim(),
          instruction: 'Return JSON with mutating calls only: applyEdit, createFile, deleteFile, renameFile.'
        }, null, 2);
        
        const mutateResponsePromise = api.ai?.performTask?.({
          promptOverride: mutatePrompt,
          systemInstruction: [
            'You are Coder AI mutation planner.',
            'Return strict JSON only.',
            'Allowed mutating calls only: applyEdit, createFile, deleteFile, renameFile.',
            'Do not return read-only calls in this pass.'
          ].join('\n'),
          context: 'writer',
          searchMode: false,
          configOverride: cfg
        ...
        try {
          const mutateResponse = await executeAiCallWithTimeout(mutateResponsePromise, 12000, 'Edit Request');
          if (!mutateResponse?.error) {
            const mutateParsed = parseAccountAiResponse(mutateResponse || {});
            calls = Array.isArray(mutateParsed.calls) ? mutateParsed.calls : [];
          }
        } catch (timeoutErr) {
          appendAccountAiMessage('tool', '⏱️  Edit request timed out. Using primary response...');
        }
      }
      
      if (!calls.length) {
        appendAccountAiMessage('tool', '✓ Analysis complete: No edits needed');
        return;
      }
      
      appendAccountAiMessage('tool', `✓ Execution plan: ${calls.length} operation(s)`);

      // 7. Execute tasks with progress
      appendAccountAiMessage('tool', `⚙️  Executing ${calls.length} task(s)...`);
      const simulator = createWorkspaceSimulator();
      const callResults = [];
      const cappedCalls = calls.slice(0, 12);

      for (let i = 0; i < cappedCalls.length; i += 1) {
        const call = cappedCalls[i];
        const fn = String(call?.function || 'unknown');
        const progress = Math.round(((i + 1) / cappedCalls.length) * 100);
        
        // Show progress with animation
        if (window.aiOutputFormatter?.formatProgress) {
          const progressHtml = window.aiOutputFormatter.formatProgress(i + 1, cappedCalls.length, `${fn}`);
          appendAccountAiMessage('tool', progressHtml);
        }

        await waitMs(ACCOUNT_AI_CALL_DELAY_MS);

        try {
          const output = await executeAccountAiCall(call, simulator);
          callResults.push({ 
            function: fn, 
            success: true, 
            output: truncateText(String(output || ''), 1500) 
          });
        } catch (error) {
          const errText = String(error?.message || error);
          if (window.aiOutputFormatter?.formatError) {
            const errorHtml = window.aiOutputFormatter.formatError(fn, errText, 'EXEC_ERROR');
            appendAccountAiMessage('tool', errorHtml);
          }
          callResults.push({ 
            function: fn, 
            success: false, 
            output: truncateText(errText, 600) 
          });
        }
      }

      // 8. Generate professional summary
      appendAccountAiMessage('tool', '📊 Generating execution summary...');
      const successCount = callResults.filter(r => r.success).length;
      const failCount = callResults.filter(r => !r.success).length;

      if (window.aiOutputFormatter?.formatExecutionSummary) {
        const summaryHtml = window.aiOutputFormatter.formatExecutionSummary({
          success: failCount === 0,
          results: callResults,
          totalTime: 0,
        ...
        appendAccountAiMessage('tool', summaryHtml);
      }

      // 9. Apply pending edits automatically
      const pendingCount = Array.isArray(state.pendingAiBatch?.actions) ? state.pendingAiBatch.actions.length : 0;
      if (pendingCount > 0) {
        appendAccountAiMessage('tool', `✨ Applying ${pendingCount} file edit(s)...`);
        try {
          await approvePendingAiChanges();
          appendAccountAiMessage('assistant', `✅ Successfully applied ${pendingCount} change(s) to your workspace`);
        } catch (applyError) {
          appendAccountAiMessage('assistant', `⚠️ Applied changes: ${pendingCount} file(s) updated. Check "Apply Staged Edits" if any issues.`);
        }
      } else {
        appendAccountAiMessage('assistant', '✓ Task completed: No file changes needed.');
      }

    } catch (error) {
      const errorMsg = error?.message || String(error);
      if (window.aiOutputFormatter?.formatError) {
        const errorHtml = window.aiOutputFormatter.formatError('AI Processing Failed', errorMsg, 'AI_ERROR');
        appendAccountAiMessage('assistant', errorHtml);
      } else {
        appendAccountAiMessage('assistant', `Error: ${errorMsg}`);
      }
    } finally {
      account.busy = false;
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) {
  ...
  async function sendAccountAiPrompt() {
    showToast('AI assistant has been removed from Coder', 'warn');
    return;
    const account = ensureAccountAiState();
    if (account.busy) return;
    ...
    try {
      ...
      appendAccountAiMessage('tool', 'Step 1/4: Collecting workspace context...');
      const workspaceSummary = await buildWorkspaceSummaryForAI();
      const indexedFiles = Array.isArray(workspaceSummary?.workspaceFiles) ? workspaceSummary.workspaceFiles.length : 0;
      appendAccountAiMessage('tool', `Step 1/4 done: Context ready (${indexedFiles} file${indexedFiles === 1 ? '' : 's'} indexed).`);
      await waitMs(ACCOUNT_AI_STEP_DELAY_MS);

      appendAccountAiMessage('tool', 'Step 2/4: Planning tool calls...');
      const systemInstruction = [
        'You are a CLI-style coding assistant running inside Coder.',
        'Work step-by-step: inspect first, then edit.',
        'Return strict JSON when tool calls are needed.',
        'Allowed functions: applyEdit, createFile, deleteFile, renameFile, getFile, listFiles, runCommand.',
        'Use getFile/listFiles/runCommand to collect data before proposing edits.',
        'For file reads, prefer narrow ranges and keep output concise.',
        'Schema: {"message":"short answer","calls":[{"function":"getFile","arguments":{"filePath":"src/app.js","rangeOrSelector":{"type":"line","startLine":1,"endLine":80}}}]}'
      ].join('\n');
      ...
      const requestPrompt = JSON.stringify({
        ...
        workspace: workspaceSummary,
        mode: 'cli-assistant',
        planningStyle: 'step-by-step',
        allowedFunctions: [...ALLOWED_AI_FUNCTIONS, 'runCommand']
      }, null, 2);
      ...
      const response = await executeAiCallWithTimeout(responsePromise, 18000, 'Planning Step');
      if (response?.error) throw new Error(String(response.error));

      const parsed = parseAccountAiResponse(response || {});
      if (parsed.message) appendAccountAiMessage('assistant', parsed.message);

      let calls = Array.isArray(parsed.calls) ? parsed.calls : [];
      if (!calls.length) {
        if (requestLikelyNeedsEdits(prompt)) {
          appendAccountAiMessage('tool', 'Step 2.5/4: No tool calls returned. Requesting concrete edit calls...');
          const mutatePrompt = JSON.stringify({
            userRequest: prompt,
            activeFile: state.activePath ? (toProjectRelative(state.activePath) || state.activePath) : null,
            workspaceFilesSample: Array.isArray(workspaceSummary?.workspaceFiles) ? workspaceSummary.workspaceFiles.slice(0, 60) : [],
            observations: String(parsed.message || response?.text || '').trim(),
            instruction: 'Return strict JSON only. Generate at least one mutating call (applyEdit/createFile/deleteFile/renameFile). No prose.'
          }, null, 2);
          const mutateResponsePromise = api.ai?.performTask?.({
            promptOverride: mutatePrompt,
            systemInstruction: [
              ...
              'Allowed mutating calls only: applyEdit, createFile, deleteFile, renameFile.',
              'Do not return read-only calls in this pass.',
              'Use exact schema with "calls": [{"function":"applyEdit","arguments":{"filePath":"...","rangeOrSelector":{"type":"full"},"newContent":"..."}}].',
              'If request asks to improve/fix/update code, you must output at least one mutating call.'
            ].join('\n'),
            context: 'writer',
            searchMode: false,
            wikiMode: false,
          ...
          try {
            ...
            if (!mutateResponse?.error) {
              const mutateParsed = parseAccountAiResponse(mutateResponse || {});
              calls = (Array.isArray(mutateParsed.calls) ? mutateParsed.calls : [])
                .filter((c) => ['applyEdit', 'createFile', 'deleteFile', 'renameFile'].includes(String(c?.function || '').trim()));
            }
          } catch (timeoutErr) {
            appendAccountAiMessage('tool', '⏱️  Edit request timed out. Using primary response...');
          }
        ...
        if (!calls.length) {
          appendAccountAiMessage('tool', 'Step 2/4 done: No actionable edit calls generated.');
          if (!parsed.message) appendAccountAiMessage('assistant', String(response?.text || 'No response'));
          return;
        }
      }
      const mutatingFns = new Set(['applyEdit', 'createFile', 'deleteFile', 'renameFile']);
      const plannedEdits = calls
        .filter((call) => mutatingFns.has(String(call?.function || '').trim()))
        .map((call) => describeAccountAiCallForFeed(call))
        .filter(Boolean)
        .slice(0, 20);
      if (plannedEdits.length) {
        appendAccountAiMessage('tool', formatAccountAiEditsFeed(plannedEdits, 'Planned edits'));
      }
      appendAccountAiMessage('tool', `Step 2/4 done: Planned ${calls.length} call(s).`);
      await waitMs(ACCOUNT_AI_STEP_DELAY_MS);

      appendAccountAiMessage('tool', 'Step 3/4: Running tool calls...');
      const simulator = createWorkspaceSimulator();
      const callResults = [];
      const cappedCalls = calls.slice(0, 12);
      for (let i = 0; i < cappedCalls.length; i += 1) {
        const call = cappedCalls[i];
        const fn = String(call?.function || 'unknown');
        appendAccountAiMessage('tool', `Running ${i + 1}/${cappedCalls.length}: ${fn}`);
        await waitMs(ACCOUNT_AI_CALL_DELAY_MS);
        try {
          const output = await executeAccountAiCall(call, simulator);
          const compact = truncateText(String(output || ''), 420);
          appendAccountAiMessage('tool', `${fn}: ${compact}`);
          callResults.push({ function: fn, success: true, output: truncateText(String(output || ''), 1500) });
        } catch (error) {
          const errText = String(error?.message || error);
          appendAccountAiMessage('tool', `${fn}: ${errText}`);
          callResults.push({ function: fn, success: false, output: truncateText(errText, 600) });
        }
      }

      appendAccountAiMessage('tool', 'Step 3/4 done: Tool execution finished.');
      await waitMs(ACCOUNT_AI_STEP_DELAY_MS);

      const hasMutatingCall = cappedCalls.some((c) => mutatingFns.has(String(c?.function || '').trim()));
      let pendingCount = Array.isArray(state.pendingAiBatch?.actions) ? state.pendingAiBatch.actions.length : 0;

      // If the first pass only inspected files but user asked for changes, run one focused mutate pass.
      if (!hasMutatingCall && pendingCount === 0 && requestLikelyNeedsEdits(prompt)) {
        appendAccountAiMessage('tool', 'Step 3.5/4: No edits yet. Requesting concrete file changes...');
        const observation = callResults.map((r) => `${r.function}: ${truncateText(r.output || '', 400)}`).join('\n');
        const mutatePrompt = JSON.stringify({
          userRequest: prompt,
          activeFile: state.activePath ? (toProjectRelative(state.activePath) || state.activePath) : null,
          observations: observation,
          instruction: 'Return strict JSON with mutating calls only (applyEdit/createFile/deleteFile/renameFile). Avoid read-only calls and return at least one mutation.'
        }, null, 2);
        const mutateResponsePromise = api.ai?.performTask?.({
          promptOverride: mutatePrompt,
          systemInstruction: [
            ...
            'Return strict JSON only.',
            'Allowed mutating calls only: applyEdit, createFile, deleteFile, renameFile.',
            'Do not return getFile/listFiles/runCommand in this pass.',
            'Use exact schema with "calls" array and canonical function names.'
          ].join('\n'),
          context: 'writer',
          searchMode: false,
          wikiMode: false,
        ...
        try {
          const mutateResponse = await executeAiCallWithTimeout(mutateResponsePromise, 12000, 'Edit Generation');
          if (!mutateResponse?.error) {
            const mutateParsed = parseAccountAiResponse(mutateResponse || {});
            const mutateCalls = Array.isArray(mutateParsed.calls) ? mutateParsed.calls : [];
            const usableMutations = mutateCalls
              .filter((c) => mutatingFns.has(String(c?.function || '').trim()))
              .slice(0, 12);
            if (usableMutations.length) {
              for (const call of usableMutations) {
                const fn = String(call?.function || 'unknown');
                try {
                  const output = await executeAccountAiCall(call, simulator);
                  appendAccountAiMessage('tool', `${fn}: ${truncateText(String(output || ''), 420)}`);
                } catch (error) {
                  appendAccountAiMessage('tool', `${fn}: ${String(error?.message || error)}`);
                }
              }
              pendingCount = Array.isArray(state.pendingAiBatch?.actions) ? state.pendingAiBatch.actions.length : 0;
            } else {
              appendAccountAiMessage('tool', 'Step 3.5/4: AI returned no valid edit calls.');
            }
          }
        } catch (timeoutErr) {
          appendAccountAiMessage('tool', '⏱️  Edit generation timed out. Proceeding with summary...');
        }
      }

      appendAccountAiMessage('tool', 'Step 4/4: Summarizing results...');
      if (pendingCount > 0 && Array.isArray(state.pendingAiBatch?.actions)) {
        const stagedLines = state.pendingAiBatch.actions
          .map((action) => formatAiActionForChat(action))
          .filter(Boolean)
          .slice(0, 30);
        if (stagedLines.length) {
          appendAccountAiMessage('tool', formatAccountAiEditsFeed(stagedLines, 'Staged edits'));
        }
      }
      const fallbackSummary = pendingCount > 0
        ? `Completed ${callResults.length} call(s). ${pendingCount} staged edit(s) are ready. Click Apply to commit changes.`
        : `Completed ${callResults.length} call(s). No staged edits pending.`;
      try {
        const finalPrompt = JSON.stringify({
          userRequest: prompt,
          executedCalls: callResults,
          pendingStagedEdits: pendingCount,
          instruction: 'Summarize what was checked, what changed, and next action.'
        }, null, 2);
        const finalResponsePromise = api.ai?.performTask?.({
          promptOverride: finalPrompt,
          systemInstruction: 'You are a concise CLI assistant. Return plain text summary only. No JSON.',
          context: 'writer',
          searchMode: false,
          wikiMode: false,
          videoMode: false,
        ...
        const finalResponse = await executeAiCallWithTimeout(finalResponsePromise, 12000, 'Summary Generation');
        const finalText = sanitizeAiVisiblePlanMessage(String(finalResponse?.text || '').trim());
        appendAccountAiMessage('assistant', finalText || fallbackSummary);
      } catch (summaryError) {
        const summaryErr = String(summaryError?.message || summaryError || '');
        if (summaryErr.includes('timeout')) {
          appendAccountAiMessage('tool', '⏱️  Summary generation timed out. Using fallback...');
        }
        appendAccountAiMessage('assistant', fallbackSummary);
      }
    } catch (error) {
      appendAccountAiMessage('assistant', `Error: ${error?.message || error}`);
    } finally {
      account.busy = false;
      if (state.sidePanelMode === 'account' && state.explorerOpen !== false) {
        renderSidePanelMode();
  ...
  // Enhanced AI message with animations
  function appendAccountAiMessageEnhanced(role, text, options = {}) {
    if (!window.aiCliEnhanced) {
      appendAccountAiMessage(role, text);
      return;
    }
    
    const messageEl = document.createElement('div');
    messageEl.className = 'ai-message-enhanced';
    ...
    if (role === 'user') {
      ...
      avatarEl.style.background = 'rgba(139,240,192,0.2)';
      avatarEl.style.borderColor = '#8df0c0';
      avatarEl.style.color = '#8df0c0';
    } else if (role === 'tool') {
      avatarEl.innerHTML = window.aiCliEnhanced.AI_SVG_WORKING;
    } else {
      avatarEl.innerHTML = window.aiCliEnhanced.AI_SVG_REVIEWING;
    }
  ...
  // Create animated plan steps visualization
  function createAnimatedPlanSteps(totalSteps) {
    const planEl = document.createElement('div');
    planEl.className = 'ai-plan-breakdown';
    planEl.id = 'ai-plan-steps';
    
    for (let i = 1; i <= totalSteps; i += 1) {
      const stepEl = document.createElement('div');
      stepEl.className = 'ai-plan-step';
      stepEl.id = `plan-step-${i}`;
      stepEl.innerHTML = `
        <span class="ai-step-number">${i}.</span>
        <div class="ai-step-content">
          <div class="ai-step-title">Step ${i}</div>
          <div class="ai-step-description" id="plan-step-${i}-desc">Initializing...</div>
          <div class="ai-step-progress" id="plan-step-${i}-progress" style="display: none;">
            ${window.aiCliEnhanced?.AI_SVG_WORKING || ''}
            <span id="plan-step-${i}-status"></span>
          </div>
        </div>
      `;
  ...
  // Update plan step with animation
  function updatePlanStep(stepNum, status, description = '', details = '') {
    const stepEl = document.getElementById(`plan-step-${stepNum}`);
    if (!stepEl) return;
    
    stepEl.className = 'ai-plan-step ' + status;
    
    const descEl = document.getElementById(`plan-step-${stepNum}-desc`);
    if (descEl) descEl.textContent = description;
    
    const progressEl = document.getElementById(`plan-step-${stepNum}-progress`);
    if (progressEl) {
      progressEl.style.display = status !== 'completed' ? 'flex' : 'none';
      const statusEl = document.getElementById(`plan-step-${stepNum}-status`);
      if (statusEl) statusEl.textContent = details;
    }
  }
  ...
  async function loadExtensionDetailFromMarketplace(options = {}) {
    ...
    try {
      ...
      const enriched = normalizeExtensionDetail({
        ...
        compatibilityMode: state.extensionDetailView?.compatibilityMode || 'controlled'
      });
      if (!enriched) return;
  ...
  async function handleSidePanelClick(event) {
    ...
    if (await handleAccountAiSideAction(action, btn)) {
      return;
    }
  ...
  function handleSidePanelInput(event) {
    ...
    if (target.id === 'account-ai-input') {
      const account = ensureAccountAiState();
      account.draft = String(target.value || '');
      scheduleSessionSave();
      return;
    }
    if (target.id === 'account-ai-provider-input') {
      const account = ensureAccountAiState();
      account.selectedProvider = String(target.value || '').trim();
      const selectedCfg = account.providers.find((p) => p.id === account.selectedProvider);
      account.selectedModel = String(selectedCfg?.model || '').trim();
      if (state.sidePanelMode === 'account') renderSidePanelMode();
      scheduleSessionSave();
      return;
    }
    if (target.id === 'account-ai-model-input') {
      const account = ensureAccountAiState();
      account.selectedModel = String(target.value || '').trim();
      scheduleSessionSave();
    }
  }
  ...
  async function handleSidePanelKeydown(event) {
    ...
    if (target.id === 'account-ai-input' && event.key === 'Enter' && !event.shiftKey) {
      event.preventDefault();
      const account = ensureAccountAiState();
      account.draft = String(target.value || '');
      await sendAccountAiPrompt();
  ...
  function normalizeExtensionCatalogEntry(raw, context = 'extensions.list') {
    ...
    return {
      ...
      compatibilityMode: safeString(raw.compatibilityMode).trim() || 'controlled',
...
${bodyHtml}
  ...
  function treeFileIconSpec(filePath) {
    ...
    if (base === 'package.json') return { color: '#f59e0b', icon: 'npm' };
    if (base === 'package-lock.json' || base.endsWith('.lock')) return { color: '#a78bfa', icon: 'lock' };
    if (base === '.gitignore' || base === '.gitattributes') return { color: '#f97316', icon: 'git' };
    if (base === '.env' || base.startsWith('.env.')) return { color: '#22c55e', icon: 'env' };
    if (base === 'readme.md') return { color: '#60a5fa', icon: 'md' };

    if (['html', 'htm'].includes(ext)) return { color: '#f97316', icon: 'html' };
    if (ext === 'css') return { color: '#3b82f6', icon: 'css' };
    if (['js', 'mjs', 'cjs'].includes(ext)) return { color: '#facc15', icon: 'js' };
    if (['ts', 'tsx'].includes(ext)) return { color: '#2563eb', icon: 'ts' };
    if (ext === 'jsx') return { color: '#06b6d4', icon: 'jsx' };
    if (ext === 'json') return { color: '#f59e0b', icon: 'json' };
    if (ext === 'md') return { color: '#60a5fa', icon: 'md' };
    if (['yml', 'yaml', 'toml', 'ini', 'cfg'].includes(ext)) return { color: '#14b8a6', icon: 'config' };
    if (ext === 'xml') return { color: '#fb7185', icon: 'xml' };
    if (ext === 'svg') return { color: '#fb923c', icon: 'svg' };
    if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'ico'].includes(ext)) return { color: '#ec4899', icon: 'image' };
    if (ext === 'pdf') return { color: '#ef4444', icon: 'pdf' };
    if (ext === 'txt') return { color: '#94a3b8', icon: 'text' };
    if (ext === 'py') return { color: '#3b82f6', icon: 'py' };
    if (ext === 'java') return { color: '#f97316', icon: 'java' };
    if (ext === 'go') return { color: '#22d3ee', icon: 'go' };
    if (ext === 'rs') return { color: '#f97316', icon: 'rust' };
    if (ext === 'c') return { color: '#60a5fa', icon: 'c' };
    if (['cpp', 'cc', 'cxx', 'c++'].includes(ext)) return { color: '#3b82f6', icon: 'cpp' };
    if (['h', 'hpp', 'hh', 'hxx'].includes(ext)) return { color: '#93c5fd', icon: 'header' };
    if (['sh', 'ps1', 'bat', 'cmd'].includes(ext)) return { color: '#84cc16', icon: 'shell' };
    if (['sql', 'db', 'sqlite'].includes(ext)) return { color: '#a78bfa', icon: 'db' };
    if (['zip', 'rar', '7z', 'tar', 'gz'].includes(ext)) return { color: '#f59e0b', icon: 'archive' };
  ...
  function treeFileIconSvg(filePath) {
    ...
    const glyphByIcon = {
      html: `<path d="M5.2 9.2 3.9 8l1.3-1.2M10.8 6.8 12.1 8l-1.3 1.2M9.1 6.4 6.9 9.6" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round" stroke-linejoin="round"/>`,
      css: `<path d="M6 6.1 5.2 10M8.8 6.1 8 10M4.8 7.4h5M4.4 9h5" fill="none" stroke="${color}" stroke-width=".9" stroke-linecap="round"/>`,
      js: `<path d="M7.5 5.9 6.4 7.9h1.2L6.5 10.2 9.5 7.7H8.3l1.1-1.8z" fill="${color}" />`,
      ts: `<path d="M6.1 6.1h3.8M8 6.1v3.9M10.6 7.5l.9.8-.9.8" fill="none" stroke="${color}" stroke-width=".9" stroke-linecap="round" stroke-linejoin="round"/>`,
      jsx: `<path d="M4.9 8h6.2M8 4.9v6.2M5.7 5.7l4.6 4.6M10.3 5.7 5.7 10.3" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round"/>`,
      json: `<path d="M6.5 6.2c-.7 0-.9.4-.9.9v.3c0 .4-.2.7-.6.9.4.2.6.5.6.9v.3c0 .5.2.9.9.9M9.5 6.2c.7 0 .9.4.9.9v.3c0 .4.2.7.6.9-.4.2-.6.5-.6.9v.3c0 .5-.2.9-.9.9" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round"/>`,
      md: `<path d="M4.9 9.9V6.1l1.6 2 1.6-2v3.8M9.8 6.1h1.4l.9 1.9.9-1.9" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round" stroke-linejoin="round"/>`,
      config: `<path d="M5.2 6.5h6M5.2 8h6M5.2 9.5h6M7 6.5v3M9.6 8v1.5" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round"/>`,
      xml: `<path d="M5.2 9.2 3.9 8l1.3-1.2M10.8 6.8 12.1 8l-1.3 1.2M8.9 6.3 7 9.7" fill="none" stroke="${color}" stroke-width=".9" stroke-linecap="round" stroke-linejoin="round"/>`,
      svg: `<circle cx="8" cy="8" r="2.7" fill="none" stroke="${color}" stroke-width=".86"/><path d="M8 5v6M5 8h6M6 6l4 4M10 6 6 10" fill="none" stroke="${color}" stroke-width=".7"/>`,
      pdf: `<path d="M5.1 9.9V6.2h1.6a1 1 0 1 1 0 2H5.1M7.8 9.9V6.2h1.3a1.3 1.3 0 0 1 0 2.6H7.8M10.8 6.2v3.7M10.8 8h1.3" fill="none" stroke="${color}" stroke-width=".78" stroke-linecap="round"/>`,
      text: `<path d="M5.2 6.4h5.6M5.2 7.9h5.6M5.2 9.4h4.2" fill="none" stroke="${color}" stroke-width=".86" stroke-linecap="round"/>`,
      py: `<path d="M6.2 6.1h2c.9 0 1 .6 1 1.1v.5H6.6c-.7 0-1.2.4-1.2 1.1v.4c0 .7.5 1.1 1.2 1.1h1.8M9.8 9.9h-2c-.9 0-1-.6-1-1.1v-.5H9.4c.7 0 1.2-.4 1.2-1.1v-.4c0-.7-.5-1.1-1.2-1.1H7.6" fill="none" stroke="${color}" stroke-width=".75" stroke-linecap="round"/>`,
      java: `<path d="M7.2 10.1h2.8a1.1 1.1 0 0 0 0-2.2H7.3v2.2M6.6 6.5c.5-.4.5-.8 0-1.2M8.2 6.5c.5-.4.5-.8 0-1.2M9.8 6.5c.5-.4.5-.8 0-1.2" fill="none" stroke="${color}" stroke-width=".76" stroke-linecap="round"/>`,
      go: `<path d="M5.2 8h5.5M8.7 6.4h1.8M8.7 9.6h1.8M5.8 6.8a1.2 1.2 0 1 0 0 2.4 1.2 1.2 0 0 0 0-2.4z" fill="none" stroke="${color}" stroke-width=".8" stroke-linecap="round"/>`,
      rust: `<circle cx="8" cy="8" r="2.1" fill="none" stroke="${color}" stroke-width=".82"/><path d="m8 4.8.5.7.9-.2.2.9.8.4-.4.8.4.8-.8.4-.2.9-.9-.2-.5.7-.5-.7-.9.2-.2-.9-.8-.4.4-.8-.4-.8.8-.4.2-.9.9.2z" fill="none" stroke="${color}" stroke-width=".66"/>`,
      c: `<path d="M9.7 6.2a2.2 2.2 0 1 0 0 3.6" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round"/>`,
      cpp: `<path d="M8.5 6.2a2.2 2.2 0 1 0 0 3.6M10 7.4h1.2M10.6 6.8v1.2M10 8.8h1.2M10.6 8.2v1.2" fill="none" stroke="${color}" stroke-width=".8" stroke-linecap="round"/>`,
      header: `<path d="M5.5 6v4M10.5 6v4M5.5 8h5" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round"/>`,
      shell: `<path d="m5 6.8 1.8 1.2L5 9.2M7.8 9.2h3.3" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round" stroke-linejoin="round"/>`,
      db: `<ellipse cx="8" cy="6.1" rx="2.8" ry="1.2" fill="none" stroke="${color}" stroke-width=".76"/><path d="M5.2 6.1v3.3c0 .65 1.25 1.2 2.8 1.2s2.8-.55 2.8-1.2V6.1" fill="none" stroke="${color}" stroke-width=".76"/>`,
      archive: `<path d="M5.1 6h5.8v4.2H5.1zM7 6v4.2M5.1 7.3h5.8" fill="none" stroke="${color}" stroke-width=".8"/>`,
      git: `<circle cx="6" cy="6.4" r=".72" fill="${color}"/><circle cx="10.2" cy="7.9" r=".72" fill="${color}"/><circle cx="6" cy="9.5" r=".72" fill="${color}"/><path d="M6.7 6.7 9.5 7.7M6 7.1v1.6" fill="none" stroke="${color}" stroke-width=".76" stroke-linecap="round"/>`,
      env: `<path d="M6 10c2.8-.1 4.3-1.8 4.7-4.4-2.5.4-4.4 1.6-4.7 4.4zM6.7 8.8 5.8 10" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round"/>`,
      npm: `<path d="M5.1 6.4h5.8v3.5h-.9V7.3h-.9v2.6h-.9V7.3h-.9v2.6h-.9z" fill="${color}"/>`,
      lock: `<rect x="5.3" y="7.4" width="5.4" height="3.2" rx=".6" fill="none" stroke="${color}" stroke-width=".82"/><path d="M6.4 7.4V6.6a1.6 1.6 0 1 1 3.2 0v.8" fill="none" stroke="${color}" stroke-width=".82"/>`,
      file: `<circle cx="8" cy="8.4" r="1.1" fill="${color}"/>`
    };

    if (icon === 'image') {
      return `
        <svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">
          <path d="M3.7 1.4h5.5l3.1 3.2V14a.8.8 0 0 1-.8.8H3.7a.8.8 0 0 1-.8-.8V2.2a.8.8 0 0 1 .8-.8z" fill="#11141d" stroke="#475569" stroke-width=".72" stroke-linejoin="round"/>
          <path d="M9.2 1.4v2.7a.8.8 0 0 0 .8.8h2.3" fill="#1a2030" stroke="#475569" stroke-width=".72" stroke-linejoin="round"/>
          <rect x="4.2" y="8.1" width="7.4" height="4.2" rx=".75" fill="${color}" opacity=".98"/>
          <path d="M4.9 11.7l1.9-1.8 1.2 1.1 1.8-2 1 1.1v1.6H4.9z" fill="#fff" opacity=".94"/>
          <circle cx="6" cy="9.3" r=".62" fill="#fff" opacity=".97"/>
        </svg>
      `;
    }
    ...
    return `
      <svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">
        <path d="M3.7 1.4h5.5l3.1 3.2V14a.8.8 0 0 1-.8.8H3.7a.8.8 0 0 1-.8-.8V2.2a.8.8 0 0 1 .8-.8z" fill="#11141d" stroke="#475569" stroke-width=".72" stroke-linejoin="round"/>
        <path d="M9.2 1.4v2.7a.8.8 0 0 0 .8.8h2.3" fill="#1a2030" stroke="#475569" stroke-width=".72" stroke-linejoin="round"/>
        ${glyphByIcon[icon] || glyphByIcon.file}
      </svg>
    `;
  }

  function tabFileIconSvg(filePath) {
    const spec = treeFileIconSpec(filePath);
    const color = spec.color || '#94a3b8';
    const icon = spec.icon || 'file';
    const compact = {
      html: `<path d="M4.7 8.9 3.5 7.8l1.2-1.1M10.6 6.7l1.2 1.1-1.2 1.1M8.9 6.2 6.9 9.4" fill="none" stroke="${color}" stroke-width="1.05" stroke-linecap="round" stroke-linejoin="round"/>`,
      css: `<path d="M5.7 6.2 5 9.4M8.2 6.2 7.5 9.4M4.6 7.3h4.6M4.2 8.6h4.6" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round"/>`,
      js: `<path d="M6.9 5.9 5.9 7.8h1l-.9 2 2.7-2.4H7.6l1-1.5z" fill="${color}"/>`,
      ts: `<path d="M5.4 6.2h3.3M7 6.2v3.2M9.6 7.2l.8.7-.8.7" fill="none" stroke="${color}" stroke-width=".95" stroke-linecap="round"/>`,
      json: `<path d="M6.1 6.2c-.6 0-.8.3-.8.8v.2c0 .4-.2.6-.5.8.3.2.5.4.5.8v.2c0 .5.2.8.8.8M9.9 6.2c.6 0 .8.3.8.8v.2c0 .4.2.6.5.8-.3.2-.5.4-.5.8v.2c0 .5-.2.8-.8.8" fill="none" stroke="${color}" stroke-width=".88" stroke-linecap="round"/>`,
      md: `<path d="M4.8 9.3V6.2l1.3 1.6 1.3-1.6v3.1M9 6.2h1.7" fill="none" stroke="${color}" stroke-width=".88" stroke-linecap="round"/>`,
      image: `<path d="M4.8 9.6 6.7 7.8l1.1 1 1.6-1.8 1 1.1M6 7.3a.7.7 0 1 1 0-.01" fill="none" stroke="${color}" stroke-width=".9" stroke-linecap="round" stroke-linejoin="round"/>`,
      pdf: `<path d="M5 9.4V6.3h1.4a.9.9 0 1 1 0 1.8H5M8 9.4V6.3h1.1a1.1 1.1 0 0 1 0 2.2H8" fill="none" stroke="${color}" stroke-width=".86" stroke-linecap="round"/>`,
      py: `<path d="M6.1 6.3h1.7c.7 0 .8.4.8.8v.4H6.3c-.6 0-1 .3-1 .9v.3c0 .6.4.9 1 .9h1.6" fill="none" stroke="${color}" stroke-width=".74" stroke-linecap="round"/>`,
      java: `<path d="M6.2 9.4h2.6a1 1 0 0 0 0-2H6.3M6.3 6.4c.4-.3.4-.6 0-.9M7.7 6.4c.4-.3.4-.6 0-.9" fill="none" stroke="${color}" stroke-width=".74" stroke-linecap="round"/>`,
      c: `<path d="M9.5 6.4a1.9 1.9 0 1 0 0 2.8" fill="none" stroke="${color}" stroke-width="1.02" stroke-linecap="round"/>`,
      cpp: `<path d="M8.4 6.4a1.9 1.9 0 1 0 0 2.8M9.8 7.6h1M10.3 7.1v1M9.8 8.6h1M10.3 8.1v1" fill="none" stroke="${color}" stroke-width=".82" stroke-linecap="round"/>`,
      file: `<circle cx="7.9" cy="8" r="1.2" fill="${color}"/>`
    };
    return `<svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">${compact[icon] || compact.file}</svg>`;
  }
  ...
  async function toggleFolderExpansion(dirPath) {
    ...
    if (!normalized) return;
    if (!state.expandedDirs.has(normalized) && isHeavyTreeDirectory(normalized)) {
      showToast(`Skipping large folder: ${basenamePath(normalized)}`, 'warning');
      return;
    }
  ...
  function isHeavyTreeDirectory(targetPath) {
    const normalized = normalizePath(targetPath);
    if (!normalized) return false;
    const parts = normalized.split('/').filter(Boolean).map((part) => part.toLowerCase());
    return parts.some((part) => TREE_HEAVY_DIRS.has(part));
  }
  ...
  function renderTabs() {
    ...
    state.tabs.forEach((filePath) => {
      ...
      fileIcon.className = 'tab-icon';
      fileIcon.innerHTML = tabFileIconSvg(filePath);
      fileIcon.title = languageLabel(file.language || detectLanguage(filePath));
      ...
      return `
        <div class="ext-feature-section">
          ...
          <div class="ext-feature-kv-grid">
            ...
            <div class="ext-feature-kv"><div class="k">Compatibility</div><div class="v">${escapeHtml(String(detail.compatibilityMode || 'controlled'))}</div></div>
  ...
  async function readDirectoryChildren(dirPath, depth) {
    ...
    const limitedEntries = entries.slice(0, TREE_MAX_ITEMS_PER_DIR);
    if (entries.length > TREE_MAX_ITEMS_PER_DIR && !warnedTreeLimitDirs.has(dirPath)) {
      warnedTreeLimitDirs.add(dirPath);
      showToast(`Large folder truncated: showing first ${TREE_MAX_ITEMS_PER_DIR} items`, 'warning');
    }

    const nodes = await Promise.all(limitedEntries.map(async (name) => {
      if (!name || name === '.' || name === '..') return null;
      const entryName = String(name);
      if (depth > 0 && TREE_IGNORED_DIRS.has(entryName.toLowerCase())) return null;
      const fullPath = joinPath(dirPath, entryName);
      const stat = await api.files?.stat?.(fullPath);
      if (!stat?.exists) return null;
      const type = stat.isDirectory ? 'dir' : 'file';
      const node = { name: entryName, path: normalizePath(fullPath), type, children: null };
      if (type === 'dir' && state.expandedDirs.has(node.path) && !isHeavyTreeDirectory(node.path)) {
        node.children = await readDirectoryChildren(node.path, depth + 1);
      } else if (type === 'dir' && state.expandedDirs.has(node.path) && isHeavyTreeDirectory(node.path)) {
  ...
  function renderWorkspaceLayout() {
    ...
    if (workspaceEl) {
      ...
      workspaceEl.style.setProperty('--ai-left-width', '0px');
    }
    ...
    document.body.classList.toggle('ai-resizing', false);
  ...
  async function handleCoderTerminalRuntimeEvent(payload) {
    ...
    if (type === 'stdout' || type === 'stderr') {
      ...
      appendCoderTerminalChunk(chunk, type);
      // Scan for errors in compilation/build output
      processTerminalOutputForErrors(chunk);
      renderCoderTerminalPanel(true);
      return;
    }
  ...
  function renderAiSidebar() {
    ...
    const showSettings = Boolean(state.aiSettingsOpen);
    const showSidebar = AI_CHAT_ENABLED || showSettings;
    renderWorkspaceLayout();
    els['btn-toggle-ai']?.classList.toggle('active', open);
    els['btn-toggle-ai']?.setAttribute('aria-hidden', AI_CHAT_ENABLED ? 'false' : 'true');
    els['ai-sidebar']?.classList.toggle('hidden', !showSidebar);
    els['ai-settings-panel']?.classList.toggle('hidden', !showSettings);
    if (els['ai-settings-panel']) {
      els['ai-settings-panel'].setAttribute('aria-hidden', showSettings ? 'false' : 'true');
    }
  }

  function toggleAiSettingsPanel(force) {
    state.aiSettingsOpen = typeof force === 'boolean' ? force : !state.aiSettingsOpen;
    if (state.aiSettingsOpen && AI_CHAT_ENABLED) state.aiOpen = true;
    renderAiSidebar();
  }
  ...
  async function openAiSettingsPanel() {
    ...
    if (AI_CHAT_ENABLED) toggleAiSidebar(true);
  ...
  function parseAiPlanResponse(response) {
    ...
    for (const call of Array.isArray(calls) ? calls : []) {
      if (!call || typeof call === 'string') continue;
      const normalized = normalizeAccountAiCall(call);
      if (!normalized.function) continue;
  ...
  async function stageAiCalls(calls) {
    ...
    for (const rawCall of calls) {
      const normalizedCall = normalizeAccountAiCall(rawCall);
      const fn = String(normalizedCall?.function || '').trim();
      const args = normalizedCall?.arguments || {};
      if (!ALLOWED_AI_FUNCTIONS.has(fn)) {
      ...
      try {
        if (fn === 'getFile') {
          const target = resolveWorkspacePath(args.path || args.filePath || '');
        ...
        if (fn === 'applyEdit') {
          const staged = await stageApplyEdit(args, simulator);
          actions.push(staged);
          continue;
        }

        if (fn === 'createFile') {
          const staged = await stageCreateFile(args, simulator);
          actions.push(staged);
          continue;
        }

        if (fn === 'deleteFile') {
          const staged = await stageDeleteFile(args, simulator);
          actions.push(staged);
          continue;
        }

        if (fn === 'renameFile') {
          const staged = await stageRenameFile(args, simulator);
          actions.push(staged);
          continue;
        }
      } catch (error) {
  ...
  async function stageApplyEdit(args, simulator) {
    ...
    if (args.newContent == null) {
      throw new Error('applyEdit requires newContent');
    }
    const replacement = String(args.newContent);
    const resolved = resolveEditRange(current, selector);
    if (resolved.kind === 'full' && replacement.trim() === '') {
      throw new Error('Blocked empty full-file overwrite');
    }
  ...
  async function approvePendingAiChanges() {
    ...
    try {
      ...
      for (const action of batch.actions) {
        if (action.type === 'applyEdit') {
          try { await api.files?.allowPath?.(action.filePath, 'file'); } catch (_) {}
        ...
        if (action.type === 'createFile') {
          const parentDir = dirnamePath(action.filePath);
          if (parentDir) {
            try { await api.files?.allowPath?.(parentDir, 'dir'); } catch (_) {}
            if (api.files?.createFolder) {
              const ensured = await api.files.createFolder(parentDir);
              if (!ensured) throw new Error(`Failed to prepare folder ${toProjectRelative(parentDir) || parentDir}`);
            }
          }
          try { await api.files?.allowPath?.(action.filePath, 'file'); } catch (_) {}
        ...
        if (action.type === 'renameFile') {
          try { await api.files?.allowPath?.(action.oldPath, 'file'); } catch (_) {}
          const nextParent = dirnamePath(action.newPath);
          if (nextParent) {
            try { await api.files?.allowPath?.(nextParent, 'dir'); } catch (_) {}
            if (api.files?.createFolder) {
              const ensured = await api.files.createFolder(nextParent);
              if (!ensured) throw new Error(`Failed to prepare folder ${toProjectRelative(nextParent) || nextParent}`);
            }
          }
          try { await api.files?.allowPath?.(action.newPath, 'file'); } catch (_) {}
  ...
  async function restoreSession() {
    try {
      ...
      state.sidePanelMode = SIDE_PANEL_MODES.includes(String(data.sidePanelMode || ''))
