(() => {
  ...
  // Error parsing/registration remains available for run output diagnostics.
  const codeErrorMap = new Map();
  ...
  function processTerminalOutputForErrors(output) {
    ...
    errors.forEach((error) => {
      ...
      registerFileErrors(error.filePath, [...existing, error]);
    });
  }
...
${bodyHtml}
  ...
  function renderLineNumbers() {
    ...
    const highlight = file?.lastLineHighlightRange && file.lastLineHighlightRange.expiresAt > Date.now()
      ? file.lastLineHighlightRange
      : null;
  ...
  function renderWorkspaceLayout() {
    ...
    workspaceEl?.classList.toggle('explorer-collapsed', !explorerOpen);
  ...
  function handleGlobalShortcuts(event) {
    ...
    if (!inProtectedInput && (event.ctrlKey || event.metaKey) && key === 'o') {
      event.preventDefault();
    ...
    if (!inProtectedInput && event.key === 'F5') {
      event.preventDefault();
      void runActiveFile();
      return;
    }

    if (!inProtectedInput && (event.ctrlKey || event.metaKey) && key === '`') {
      event.preventDefault();
      void openCoderTerminalPanel();
      return;
    }

    if (!inProtectedInput && (event.ctrlKey || event.metaKey) && key === 'b') {
      event.preventDefault();
      toggleExplorerPane();
    }
  }
