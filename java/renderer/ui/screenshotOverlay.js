...
export class ScreenshotOverlay {
  ...
  init() {
    if (!this.layer || !this.canvas || !this.ctx) {
      console.warn('[ScreenshotOverlay] Required screenshot DOM nodes are missing; overlay disabled.');
      return;
    }
  ...
  handleKeyPress(e) {
    ...
    if (e.key === 'Shift' && this.layer.classList.contains('active') && !this.isWindowPickerActive && !this.isDisplayPickerActive) {
      if (this.selectionRect && this.selectionRect.w > 10 && this.selectionRect.h > 10) {
        e.preventDefault();
        this.downloadSelection();
      } else if (this.lensMode) {
        e.preventDefault();
        this.showToast('Drag to select an area first');
      }
      return;
    }

    if (e.key === 'Enter' && this.layer.classList.contains('active') && !this.isWindowPickerActive && !this.isDisplayPickerActive) {
      if (this.selectionRect && this.selectionRect.w > 10 && this.selectionRect.h > 10) {
        e.preventDefault();
        this.captureSelection(this.selectionRect.x, this.selectionRect.y, this.selectionRect.w, this.selectionRect.h);
      } else if (this.lensMode) {
        e.preventDefault();
        this.showToast('Drag to select an area first');
      }
  ...
  start(nativeImage, onCaptureOverride = null, options = {}) {
    this.overrideCapture = onCaptureOverride;
    this.cancelDelayCapture(false);
    this.selectionRect = null;
    this.lensMode = !!options.lensMode;
    this.captureBounds = options.captureBounds || null;
    const dataUrl = nativeImage.toDataURL();
    this.sourceImage = new Image();
    this.sourceImage.onload = () => {
      this.layer.classList.add('active');
      this.layer.classList.toggle('lens-mode', this.lensMode);
  ...
  hide() {
    ...
    this.layer.classList.remove('lens-mode');
  ...
  startDelayCapture(delaySeconds, captureFn, onCaptureOverride = null, options = {}) {
    ...
    if (!delaySeconds || delaySeconds <= 0) {
      this.delayCaptureFn()
        .then(img => this.start(img, onCaptureOverride, options))
        .catch((err) => {
  ...
  onMouseUp(e) {
    ...
    if (w > 10 && h > 10) {
      ...
      this.render();
      if (this.lensMode) {
        this.showToast('Press Enter to search this area with Google Lens');
      } else {
  ...
  async onContextMenu(e) {
    if (!this.layer.classList.contains('active')) return;
    if (!this.selectionRect || this.selectionRect.w <= 10 || this.selectionRect.h <= 10) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  async downloadSelection() {
    if (!this.selectionRect || this.selectionRect.w <= 10 || this.selectionRect.h <= 10) return;
    try {
      const dataUrl = this.captureSelectionToDataUrl(
      ...
      if (!dataUrl) {
        this.showToast('No selected area to save');
        return;
      }
      if (window.browserAPI?.downloads?.start) {
        this.hide();
        await window.browserAPI.downloads.start(dataUrl, {
          saveAs: true,
          filename: `omx-capture-${Date.now()}.png`
        });
        this.showToast('Selected capture ready to download');
        return;
      }
      this.showToast('Download unavailable');
    } catch (error) {
      console.error('[Screenshot] Save selection failed:', error);
      this.showToast('Failed to save selected capture');
    }
  }

  render() {
    if (!this.sourceImage) return;
    const captureRect = this.captureBounds || { left: 0, top: 0, width: this.canvas.width, height: this.canvas.height };

    // Clear
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);

    // Lens mode keeps the app visible and only draws the selection box.
    if (!this.lensMode) {
      this.ctx.drawImage(this.sourceImage, captureRect.left, captureRect.top, captureRect.width, captureRect.height);
      this.ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
      this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
    }

    // 3. Draw active or completed selection
    const rect = this.isSelecting
      ? {
          x: Math.min(this.startPos.x, this.currentPos.x),
          y: Math.min(this.startPos.y, this.currentPos.y),
          w: Math.abs(this.currentPos.x - this.startPos.x),
          h: Math.abs(this.currentPos.y - this.startPos.y)
        }
      : this.selectionRect;

    if (rect && rect.w > 0 && rect.h > 0) {
      const { x, y, w, h } = rect;
      const relativeX = x - captureRect.left;
      const relativeY = y - captureRect.top;
      const scaleX = this.sourceImage ? (this.sourceImage.width / captureRect.width) : 1;
      const scaleY = this.sourceImage ? (this.sourceImage.height / captureRect.height) : 1;
      const sx = Math.round(relativeX * scaleX);
      const sy = Math.round(relativeY * scaleY);
      const sw = Math.round(w * scaleX);
      const sh = Math.round(h * scaleY);

      if (!this.lensMode) {
        // Redraw source image cleanly inside the rect
        this.ctx.drawImage(this.sourceImage, sx, sy, sw, sh, x, y, w, h);
      }
      
      // Border
      this.ctx.strokeStyle = this.lensMode ? 'rgba(255, 255, 255, 0.96)' : '#fff';
      this.ctx.lineWidth = this.lensMode ? 2 : 1;
      this.ctx.setLineDash(this.lensMode ? [] : [5, 3]);
      this.ctx.strokeRect(x, y, w, h);
      this.ctx.setLineDash([]);

      if (this.lensMode) {
        // Subtle dark outline for contrast on bright pages.
        this.ctx.strokeStyle = 'rgba(0, 0, 0, 0.35)';
        this.ctx.lineWidth = 1;
        this.ctx.strokeRect(x + 0.5, y + 0.5, Math.max(0, w - 1), Math.max(0, h - 1));
      }
    }
  }

  captureSelectionToDataUrl(x, y, w, h) {
    // Crop logic mapped from viewport coordinates back to capture coordinates.
    const temp = document.createElement('canvas');
    const captureRect = this.captureBounds || { left: 0, top: 0, width: this.canvas.width, height: this.canvas.height };

    // Map viewport coordinates back to the captured image coordinates.
    const relativeX = x - captureRect.left;
    const relativeY = y - captureRect.top;
    const scaleX = this.sourceImage ? (this.sourceImage.width / captureRect.width) : 1;
    const scaleY = this.sourceImage ? (this.sourceImage.height / captureRect.height) : 1;
  ...
  captureSelection(x, y, w, h) {
    const dataUrl = this.captureSelectionToDataUrl(x, y, w, h);
    this.finish(dataUrl);
  }
