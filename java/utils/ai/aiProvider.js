...
// Eliza bot provider (1966 DOCTOR script)
const elizaProvider = require('./elizaProvider');
...
async function performTask(input, options = {}) {
  ...
  if (provider === 'eliza') {
    // delegate to the dedicated Eliza provider module so that
    // randomization, memory sizing, and enhanced fallback logic are
    // respected.  We forward the original options object exactly so
    // callers can control behaviour via aiConfig or explicit flags.
    const elizaResult = await elizaProvider.performTask(input, options);
    // ensure provider field is set correctly
    if (elizaResult && !elizaResult.provider) elizaResult.provider = 'eliza';
    return elizaResult;
  }
