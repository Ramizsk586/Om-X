...
export class QuickBot {
  ...
  static formatResponse(input, baseResponse, confidence = 0.7) {
    const raw = String(baseResponse || '').trim();
    if (!raw) return '';
    return this.composeAnswer(input, 'local', raw, confidence) || raw;
  }
  static findMatch(input) {
    ...
    // 3) Greeting shortcut - More flexible greeting detection
    for (const start of this._greetingStarts) {
      // Check for: "hello", "hello ", "hello world", or "hello?" etc.
      if (cleanInput === start || cleanInput.startsWith(start + " ") || cleanInput.startsWith(start + "?")) {
