...
const state = {
  ...
  nextWatcherId: 1,
  configurationByWindow: new Map(), // windowId -> configuration object
    ...
    getConfiguration(section) {
      ...
      return {
        get(key, def) {
          ...
          const workspaceConfig = state.configurationByWindow.get(windowId) || {};
          ...
          if (fullKey) {
            ...
            for (const k of keys) {
              if (!k || typeof value !== 'object') break;
              value = value[k];
            }
            if (value !== undefined) return cloneConfigValue(value);
          }
...
async function handleReq(method, params = {}) {
  switch (method) {
    ...
    case 'host.configurationChanged': {
      const windowId = Number(params.windowId) || 0;
      const configuration = params.configuration || {};
      if (windowId) {
        state.configurationByWindow = state.configurationByWindow || new Map();
        state.configurationByWindow.set(windowId, configuration);
      }
      const eventBus = windowId ? getWindowEventBus(windowId) : null;
      if (eventBus?.onDidChangeConfiguration) {
        eventBus.onDidChangeConfiguration.fire({
